...
if [[ ${NODE_VERSION} == "12.22.12" && ( ${PROJECT} != "gapic-node-templating" || ${PROJECT} != "node-bootstrap-container" || ${PROJECT} != "typeless-sample-bot" ) ]]; then
...
units)
    ...
    if [[ ${PROJECT} != "typeless-sample-bot" ]] || [[ ${PROJECT} != "gapic-node-templating" ]] || [[ ${PROJECT} != "node-bootstrap-container" ]] || [[ ${NODE_VERSION} != "12.22.12" ]]; then
        npm test
        retval=$?
    fi
    ;;
