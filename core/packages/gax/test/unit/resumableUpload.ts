...
interface BufferSourceFixture {
  source: gax.ResumableSource;
  streams: Readable[];
}

function bufferSource(payload: Buffer): BufferSourceFixture {
  ...
  return {
    source: {
      ...
      getStream: (offset?: number) => {
        ...
        const stream = Readable.from([payload.subarray(start)]);
        streams.push(stream);
        return stream;
      },
    },
...
describe('resumable upload', () => {
  ...
  it('resumes from a session URL by opening the source at the offset', async () => {
    const requests: MockRequestOptions[] = [];
    const auth = mockAuth(async opts => {
      requests.push(opts);
      const command = commandOf(opts);
  ...
  it('aborts when the global deadline is exceeded', async () => {
    ...
    await helper.start({
      uploadSource: bufferSource(Buffer.alloc(10)).source,
      globalDeadlineMs: 5,
  ...
  it('rejects with DEADLINE_EXCEEDED when the stream stalls past the deadline', async () => {
    ...
    const source: gax.ResumableSource = {
      size: 0,
      getStream: () => stalled,
    };
    await helper.start({
  ...
  it('cancels an in-flight upload and notifies the server', async () => {
    ...
    await helper.start({uploadSource: source, chunkSize: GRANULARITY});
  ...
  it('resolves the GAPIC method call with a ResumableUploadSession object', async () => {
    const auth = mockAuth(() => {
    ...
    assert.ok(result[0] instanceof gax.ResumableUploadSession);
    assert.strictEqual(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  it('exposes the granularity-rounded chunk size on the session', async () => {
    const auth = mockAuth(async opts => {
      const command = commandOf(opts);
      if (command === 'start') {
        return resumableUploadResponse(200, {
    ...
    const helper = new gax.ResumableUploadSession(buildContext(auth));
    await helper.start({
      uploadSource: bufferSource(Buffer.alloc(GRANULARITY + 1)).source,
      chunkSize: 3.5 * GRANULARITY,
    });
    assert.strictEqual(helper.chunkSize, 3 * GRANULARITY);
    await helper.finished();
  });

  it('reconciles a start response that is missing X-Goog-Upload-Status', async () => {
    const requests: MockRequestOptions[] = [];
    const auth = mockAuth(async opts => {
      requests.push(opts);
      const command = commandOf(opts);
    ...
    assert.deepStrictEqual(
      requests.map(r => commandOf(r)),
      ['start', 'query', 'upload', 'upload, finalize'],
    );
  });

  it('treats a missing status header during recovery as fatal', async () => {
    const requests: MockRequestOptions[] = [];
    const auth = mockAuth(async opts => {
      requests.push(opts);
      const command = commandOf(opts);
    ...
    await assert.rejects(helper.finished(), /header while in recovery/i);
    assert.deepStrictEqual(
      requests.map(r => commandOf(r)),
      ['start', 'upload', 'query'],
    );
  ...
  it('sends start headers only on the start request and applies timeouts everywhere', async () => {
    const requests: MockRequestOptions[] = [];
    const auth = mockAuth(async opts => {
      requests.push(opts);
      const command = commandOf(opts);
    ...
    await helper.start({
      ...
      chunkSize: GRANULARITY,
      timeout: 4321,
      startHeaders: {'developer-token': 'token-123'},
    });
    await helper.finished();

    assert.strictEqual(requests[0].headers!['developer-token'], 'token-123');
    for (const request of requests.slice(1)) {
      assert.strictEqual(request.headers!['developer-token'], undefined);
      assert.strictEqual(request.timeout, 4321);
    }
  });

  it('recovers from a stalled upload request by querying and reopening the source', async () => {
    const requests: MockRequestOptions[] = [];
    let uploadAttempts = 0;
    const auth = mockAuth(opts => {
      requests.push(opts);
        ...
        if (uploadAttempts === 1) {
          return new Promise<MockResponse>((resolve, reject) => {
            opts.signal?.addEventListener('abort', () => {
              reject(new Error('aborted by stall detection'));
            });
            // Keep `resolve` referenced so the promise stays pending until
            // the stall timer aborts the request.
            void resolve;
          });
        }
        return resumableUploadResponse(200, {'x-goog-upload-status': 'active'});
    ...
    const payload = Buffer.alloc(GRANULARITY + 1);
    const fixture = bufferSource(payload);
    const helper = new gax.ResumableUploadSession(buildContext(auth));
    await helper.start({
      uploadSource: fixture.source,
    ...
    assert.ok(fixture.streams.length >= 2);
  });

  it('rejects resumption when the source cannot be re-opened at the offset', async () => {
    const auth = mockAuth(async opts => {
      const command = commandOf(opts);
      if (command === 'query') {
        return resumableUploadResponse(200, {
    ...
    const source: gax.ResumableSource = {
      ...
      getStream: (offset = 0) => {
        if (offset > 0) {
          throw new Error('source cannot seek');
        }
        return Readable.from([payload.subarray(offset)]);
      },
    };
    const helper = new gax.ResumableUploadSession(buildContext(auth));
    await helper.start({
