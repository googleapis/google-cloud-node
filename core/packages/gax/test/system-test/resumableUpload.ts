...
class CrashingStream extends Readable {
  ...
  constructor(data: Buffer, limit: number, start = 0) {
    super();
    this.data = start >= limit ? Buffer.alloc(0) : data.subarray(start);
    this.limit = Math.max(limit - start, 0);
  }
...
describe('resumable upload (system)', () => {
  ...
  it('uploads a payload over HTTP and completes the session', async () => {
    const helper = new gax.ResumableUploadSession(context);
  ...
  it('recovers from a simulated crash using a saved session URL', async () => {
    ...
    const crashed = new gax.ResumableUploadSession(context);
    const crashSource: gax.ResumableSource = {
      size: data.length,
      getStream: (offset = 0) =>
        new CrashingStream(data, 2 * CHUNK_SIZE, offset),
    };
    await crashed.start({
      uploadSource: crashSource,
      chunkSize: CHUNK_SIZE,
    });
    ...
    const resumed = new gax.ResumableUploadSession(context);
