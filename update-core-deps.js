...
function scanDirForPackages(baseDir) {
  ...
  for (const dir of dirs) {
    if (dir === 'gaxios') continue; // Skip gaxios completely
...
delete targetLibraries['gaxios']; // Just to be extra safe
...
for (const pattern of searchPatterns) {
  ...
  for (const relPath of files) {
    // Explicitly ignore gaxios files
    if (relPath.includes('/gaxios/')) {
      console.log(`Skipping ${relPath} completely.`);
      continue;
    }
