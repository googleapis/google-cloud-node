 ...
 *
 * @param {object=} options - MachineTypes search options.
 * @param {boolean} options.autoPaginate - Have pagination handled
 *     automatically. Default: true.
 * @param {string} options.filter - Search filter in the format of
 *     `{name} {comparison} {filterString}`.
 ...
 * @param {function} callback - The callback function.
 * @param {?error} callback.err - An error returned while making this request.
 * @param {module:compute/MachineTypes} callback.machineTypes - MachineType
 *     objects from your project.
 * @param {?object} callback.nextQuery - If present, query with this object to
 *     check for more results.
 * @param {object} callback.apiResponse - The full API response.
 ...
 *   // `machineTypes` is an array of `getMachineTypes` objects.
 * });
 *
 * //-
 * // To control how many API requests are made and page through the results
 ...
 *
 * //-
 * // Get the machineTypes from your project as a readable object stream.
 * //-
 * gce.getMachineTypes()
 *   .on('error', console.error)
 *   .on('data', function(machineType) {
 *     // `machineType` is a `getMachineTypes` object.
 *   })
 *   .on('end', function() {
 *     // All machineTypes retrieved.
 *   });
 *
 * //-
 * // If you anticipate many results, you can end a stream early to prevent
