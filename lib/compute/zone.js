...
Zone.prototype.getInstances = function(options, callback) {
  ...
  var self = this;
  this.makeReq_('GET', '/instances', query, null, function(err, resp) {
    if (err) {
      callback(err);
      return;
    }
    if (resp.items) {
      ...
      for (var i = 0; i < resp.items.length; i++) {
        ...
        var instance = self.instance(metadata.name);
...
Zone.prototype.getDisks = function(options, callback) {
  ...
  var self = this;
  this.makeReq_('GET', '/disks', query, null, function(err, resp) {
    if (err) {
      callback(err);
      return;
    }
    if (resp.items) {
      ...
      for (var i = 0; i < resp.items.length; i++) {
        var metadata = resp.items[i];
        var disk = self.disk(metadata.name);
