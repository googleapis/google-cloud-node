 ...
 * var myRegion = compute.region('region-name');
 *
 * var operation = myRegion.operation('operation-name');
 */
 ...
 *
 * @example
 * var callback = function(err, address, operation) {
...
Region.prototype.createAddress = function(name, callback) {
  ...
  this.makeReq_('POST', '/addresses', null, body, function(err, resp) {
    ...
    callback(null, address, operation);
  });
};
...
/**
 * Get a list of operations for this region. For a detailed description of
 * method's options see [API reference](https://goo.gl/saRUxf).
 *
 ...
 *   if (nextQuery) {
 *     // More results exist.
 *     myRegion.getOperations(nextQuery, callback);
 *   }
 * };
 *
 * myRegion.getOperations({
