 ...
 *
 * @param {string} zone - the compute engine zone
 * @param {string} machineType - new machine type.
 * @param {function=} callback - The callback function.
 * @param {?error} callback.err - An error returned while making this request.
 * @param {module:compute/operation} callback.operation - An operation object
 *     that can be used to check the status of the request.
 ...
 * });
 */
VM.prototype.setMachineType = function(zone, machineType, callback) {
  var self = this;

  callback = callback || util.noop;

  var machineTypeUrl = 'zones/' + zone + '/machineTypes/' + machineType;

  self.request({
    method: 'POST',
    uri: '/setMachineType',
    json: { 'machineType': machineTypeUrl }
  }, callback);
};
