...
Transaction.prototype.get = function(keys, callback) {
  ...
  this.makeReq(
      'lookup',
      new pb.LookupRequest(req),
      pb.LookupResponse, function(err, resp) {
    if (err) {
      return callback(err);
    }
...
Transaction.prototype.save = function(entities, callback) {
  ...
  this.makeReq(
      'commit',
      new pb.CommitRequest(req),
      pb.CommitResponse, function(err, resp) {
    if (err || !resp) {
      return callback(err);
    }
...
Transaction.prototype.runQuery = function(q, callback) {
  ...
  this.makeReq('runQuery',
    new pb.RunQueryRequest(req),
    pb.RunQueryResponse,
    function(err, resp) {
