 ...
 * @param {string=} namespace - Namespace to run transaction operations at. If
 *     no namespace is provided, the default namespace is used.
 *
 * @example
 * ```js
 ...
 * ```
 */
function Transaction(conn, datasetId, namespace) {
  this.conn = conn;
 ...
 * transaction.save([
 *   {
 *     key: ds.key('Company', 123),
 *     data: {
 *       HQ: 'Dallas, TX'
 *     }
 *   },
 *   {
 *     key: ds.key('Product', 'Computer'),
 *     data: {
 *       vendor: 'Dell'
 *     }
 *   }
 ...
 * transaction.delete(ds.key('Company', 123), function(err) {});
 *
