...
function keyFromKeyProto(proto) {
  ...
  var key = new Key(proto.partitionId.datasetId, path.kind);
  key.keyNS = proto.partitionId.namespace;
  key.keyId = path.id;
  key.keyName = path.name;
  return key;
}

function keyToKeyProto(datasetId, key) {
  var p = { kind: key.kind };
  if (key.keyId) {
    p.id = key.keyId;
  }
  ...
  return {
    partitionId: { datasetId: datasetId, namespace: key.keyNS },
    path: [p]
  };
}
