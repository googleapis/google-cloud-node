...
/** @const {object} A mapping of native type to Datastore type. */
var NATIVE_TO_DATASTORE_TYPE = {
  array: 'list_value',
  boolean: 'boolean_value',
  buffer: 'blob_value',
  date: 'timestamp_microseconds_value',
  double: 'double_value',
  int: 'integer_value',
  key: 'key_value',
  object: 'entity_value',
...
function valueToProperty(v) {
  ...
  if (util.is(v, 'array')) {
    p.list_value = v.map(valueToProperty);
    return p;
  }
  if (v instanceof Key) {
  ...
  if (util.is(v, 'object') && Object.keys(v).length > 0) {
    var property = [];
...
function registerKind(namespace, kind, fieldMeta) {
  ...
  var schema = Object.keys(fieldMeta).reduce(function(acc, fieldName) {
    validateField(fieldName, fieldMeta[fieldName]);
    acc[fieldName] = {
      metadata: fieldMeta[fieldName],
  ...
  entityMeta[namespace][kind] = schema;
}
...
/**
 * Return a null-valued protocol. Useful for creating a schema, such as when
 * registering a Kind.
 *
 * @private
 *
 ...
 *
 * @example
 * metadataTypeToProto({ type: String, multi: true });
 * // {
 * //   list_value: [
 * //     string_value: null
 * //   ]
 * // }
 */
function metadataTypeToProto(metadataType) {
  if (util.is(metadataType.type, 'object')) {
    return {
      entity_value: {
        property: Object.keys(metadataType.type).map(function(type) {
            return {
              name: type,
              value: metadataTypeToProto(metadataType.type[type])
            };
          })
      }
    };
  ...
  var proto = {};
  var name;
  if (metadataType.type.name) {
    name = metadataType.type.name.toLowerCase();
  } else {
    name = metadataType.type.toLowerCase();
  }
  if (metadataType.multi) {
    var arrayObj = {};
    arrayObj[NATIVE_TO_DATASTORE_TYPE[name]] = null;
    proto.list_value = [ arrayObj ];
  } else {
    proto[NATIVE_TO_DATASTORE_TYPE[name]] = null;
  }
  return proto;
}

/**
 * Validate an entity against a Kind's registered schema.
 *
 * @param {string} namespace - Namespace.
 * @param {string} kind - Name of the kind to compare against.
 * @param {object} entity - An entity to compare with.
 * @return {boolean)
 */
function validateKind(namespace, kind, entity) {
  kind = getKind(namespace, kind);
  entity = entityToEntityProto(entity);
  var LIST_VALUE = NATIVE_TO_DATASTORE_TYPE.array;
  var ENTITY_VALUE = NATIVE_TO_DATASTORE_TYPE.object;

  function getProperty(properties, name) {
    return properties.filter(function(entity) {
  ...
  function validateEntityValue(proto, entity) {
    return proto[ENTITY_VALUE].property.every(function(item) {
      var protoType = getPropertyType(item.value);
      var property = getProperty(entity[ENTITY_VALUE].property, item.name);
      if (!property) {
        return false;
      }
      var propertyType = getPropertyType(property.value);
      if (protoType === ENTITY_VALUE) {
        return validateEntityValue(item.value, property.value);
      }
      return protoType === propertyType;
    });
  }

  function validateListValue(proto, entity) {
    return proto[LIST_VALUE].every(function(item) {
      var itemType = getPropertyType(item);
      return entity[LIST_VALUE].every(function(value) {
        return getPropertyType(value) === itemType;
      });
    });
  }

  return Object.keys(kind).every(function(propertyName) {
    var proto = kind[propertyName].protocol;
    var protoType = getPropertyType(proto);
    var entityProperty = getProperty(entity.property, propertyName);
    if (!entityProperty) {
      return false;
    }
    if (protoType === ENTITY_VALUE) {
      return validateEntityValue(proto, entityProperty.value);
    }
    if (protoType === LIST_VALUE) {
      return validateListValue(proto, entityProperty.value);
    }
    return protoType === getPropertyType(entityProperty.value);
  });
}

module.exports.validateKind = validateKind;
 ...
 * @param {*} field.type - Field data type.
 *
 * @example
 * validateField('title', {
 *   type: String
...
function validateField(name, field) {
  ...
  if (typeof field.type !== 'object' &&
      PRIMITIVE_KINDS.indexOf(field.type) === -1 &&
      field.type.name &&
      PRIMITIVE_KINDS.indexOf(field.type.name.toLowerCase()) === -1) {
    throw new Error('Unknown kind for field ' + name);
  }
  if (typeof field.type === 'object') {
    Object.keys(field.type).forEach(function(type) {
      validateField(type, field.type[type]);
    });
  }
}
