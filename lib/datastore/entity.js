...
var keyFromKeyProto = function(proto) {
  var path = [];
  if (proto.partition_id.namespace) {
...
function propertyToValue(property) {
  ...
  if (property.timestamp_microseconds_value !== null) {
    return new Date(property.timestamp_microseconds_value);
  }
...
function valueToProperty(v) {
  ...
  if (v instanceof Date) {
    p.timestamp_microseconds_value = v.getTime();
    return p;
  }
...
var queryToQueryProto = function(q) {
  ...
  // pagination
  if (q.startVal) {
    query.start_cursor = q.startVal;
  }
  if (q.endVal) {
    query.end_cursor = q.endVal;
  }
  if (q.offsetVal > 0) {
