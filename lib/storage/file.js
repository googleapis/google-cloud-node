 ...
 *
 * @param {object} options - Configuration object.
 * @param {object} options.expiration - Timestamp (seconds since epoch) when this
 *     policy will expire.
...
File.prototype.getSignedPolicy = function(options, callback) {
  ...
  console.log("New signed policy called");

  var expirationString = new Date(options.expiration).toISOString();
  var conditions = []; 

  // Populate equals conditions
  for (var equalConstraint in options.equals) {
    if (!(equalConstraint instanceof Array) || equalsConstraint.length != 2) {
      throw new Error('Equals constraint must be an array of two elements');
    } else {
      conditions.push(['eq', equalConstraint[0], equalConstraint[1]]);
    }
  }

  // Populate starts with conditions
  for (var startConstraint in options.startsWith) {
    if (!(startConstraint instanceof Array) || startConstraint.length != 2) {
      throw new Error('StartsWith constraint must be an array of two elements');
    } else {
      conditions.push(['startsWith', startConstraint[0], startConstraint[1]]);
    }
  }
