...
WriteStream.prototype.write = function(chunk, encoding, callback) {
  this._writeToRemoteStream(chunk);
};
...
WriteStream.prototype._writeToRemoteStream = function(data) {
  var that = this;
  if (!this.metadataWritten) {
    ...
    this.remoteStream.write('\n\n');
    this.remoteStream.write('--foo_bar_baz\n');
    this.remoteStream.write('Content-Type: ' + this.metadata.contentType + '\n');
    this.metadataWritten = true;
  }
  // dump file contents.
  this.remoteStream.write(data);
