...
BigQuery.prototype.query = function(options, callback) {
  ...
  function runQuery() {
    ...
    function responseHandler(err, resp) {
      ...
      var nextQuery = null, rows = null;
      if (resp.jobComplete === false) {
        // Query is still running.
        nextQuery = extend({
          job: that.job(options.job.id)
        }, options);
      } else if (resp.pageToken) {
        // More results exist.
        nextQuery = extend({
          job: that.job(resp.jobReference.jobId)
        }, options, {
      ...
      onComplete(null, rows, nextQuery, resp.totalRows);
    }

    function onComplete(err, rows, nextQuery, totalRows) {
      if (err) {
        if (stream) {
      ...
      if (stream) {
        ...
        callback(null, rows, nextQuery, totalRows);
      }
    }
  }
};
