...
Subscription.prototype.startPulling_ = function() {
  var self = this;
  if (this.closed ||
    (this.maxInProgress !== Infinity && this.inProgress >= this.maxInProgress)){
    return;
  }
  this.pull({
    returnImmediately: false,
    maxResults: (this.maxInProgress !== Infinity ?
    this.maxInProgress - this.inProgress : null)
...
/**
 * Mark message as done (without acknowledge to the backend that the message was retrieved).
 */
Subscription.prototype.done = function() {
  if(this.maxInProgress !== Infinity && this.inProgress-- >= this.maxInProgress){
    this.startPulling_.bind(this)();
  }
};
