 ...
 * @param {Number} options.maxInProgress - Maximum messages in progress
 */
/**
...
function Subscription(pubsub, options) {
  ...
  this.maxInProgress = util.is(options.maxInProgress, 'number') ? options.maxInProgress : Infinity;
  this.inProgress = 0;
...
Subscription.prototype.startPulling_ = function() {
  var self = this;
  if (this.closed || (this.maxInProgress != Infinity && this.inProgress >= this.maxInProgress)) {
    return;
  }
  this.pull({
    returnImmediately: false,
    maxResults: (this.maxInProgress != Infinity ? this.maxInProgress - this.inProgress : null)
...
Subscription.prototype.ack = function(ackIds, callback) {
  ...
  this.makeReq_('POST', path, null, body, function () {
    if (self.maxInProgress != Infinity && self.inProgress-- >= self.maxInProgress) {
      self.startPulling_.bind(self)();
    }

    (callback || util.noop).apply(self, arguments);
  });
};
...
Subscription.prototype.pull = function(options, callback) {
  ...
  this.makeReq_('POST', path, null, body, function(err, response) {
    ...
    this.inProgress += messages.length;
