 ...
 * @return {Subscription}
 */
Connection.prototype.subscribe = function(opts) {
  var that = this;
  var subscription = {
  ...
  var sub = new Subscription(that, subscription.name);
  sub.autoAck = !!opts.autoAck;
  this.makeReq('POST', 'subscriptions', null, subscription, function(err, item) {
    // TODO(jbd): maybe init a subscription instance if http 200 or 409.
    // TODO(jbd): subscriptions are not mutable, return error and provide
    // a way to create and get a subscription. (or upsert a subscription).
    if (err && err.code != 409) {
      sub.emitError_(err);
      return;
    }
