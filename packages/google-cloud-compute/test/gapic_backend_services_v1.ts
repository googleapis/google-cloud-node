...
describe('v1.BackendServicesClient', () => {
  ...
  describe('Common methods', () => {
    ...
    it('does not allow setting both universeDomain and universe_domain', () => {
      assert.throws(() => {
        new backendservicesModule.v1.BackendServicesClient({
          universe_domain: 'example.com',
  ...
  describe('getEffectiveSecurityPolicies', () => {
    it('invokes getEffectiveSecurityPolicies without error', async () => {
      ...
      client.innerApiCalls.getEffectiveSecurityPolicies =
        stubSimpleCall(expectedResponse);
      const [response] = await client.getEffectiveSecurityPolicies(request);
      assert.deepStrictEqual(response, expectedResponse);
      const actualRequest = (
        client.innerApiCalls.getEffectiveSecurityPolicies as SinonStub
      ).getCall(0).args[0];
      assert.deepStrictEqual(actualRequest, request);
      const actualHeaderRequestParams = (
        client.innerApiCalls.getEffectiveSecurityPolicies as SinonStub
    ...
    it('invokes getEffectiveSecurityPolicies without error using callback', async () => {
      ...
      const promise = new Promise((resolve, reject) => {
        client.getEffectiveSecurityPolicies(
          ...
          (
            err?: Error | null,
            result?: protos.google.cloud.compute.v1.IGetEffectiveSecurityPoliciesBackendServiceResponse | null,
          ) => {
            if (err) {
              reject(err);
            } else {
    ...
    it('invokes getEffectiveSecurityPolicies with error', async () => {
      ...
      const expectedHeaderRequestParams = `project=${defaultValue1 ?? ''}&backend_service=${defaultValue2 ?? ''}`;
      const expectedError = new Error('expected');
      client.innerApiCalls.getEffectiveSecurityPolicies = stubSimpleCall(
        undefined,
        expectedError,
      );
      await assert.rejects(
        client.getEffectiveSecurityPolicies(request),
        expectedError,
      );
      const actualRequest = (
        client.innerApiCalls.getEffectiveSecurityPolicies as SinonStub
