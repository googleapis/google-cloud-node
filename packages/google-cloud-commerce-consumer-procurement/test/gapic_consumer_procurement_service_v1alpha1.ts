...
describe('v1alpha1.ConsumerProcurementServiceClient', () => {
    ...
    it('invokes getOrder without error using callback', async () => {
      ...
      const promise = new Promise((resolve, reject) => {
        client.getOrder(
          ...
          (
            err?: Error | null,
            result?: protos.google.cloud.commerce.consumer.procurement.v1alpha1.IOrder | null
          ) => {
            if (err) {
              reject(err);
            } else {
  ...
  describe('placeOrder', () => {
    it('invokes placeOrder without error', async () => {
      ...
      assert.deepStrictEqual(response, expectedResponse);
      const actualRequest = (
        client.innerApiCalls.placeOrder as SinonStub
      ).getCall(0).args[0];
      assert.deepStrictEqual(actualRequest, request);
      const actualHeaderRequestParams = (
        client.innerApiCalls.placeOrder as SinonStub
    ...
    it('invokes placeOrder with call error', async () => {
      ...
      const expectedHeaderRequestParams = `parent=${defaultValue1 ?? ''}`;
      const expectedError = new Error('expected');
      client.innerApiCalls.placeOrder = stubLongRunningCall(
        undefined,
        expectedError
      );
    ...
    it('invokes listOrdersStream without error', async () => {
      ...
      const promise = new Promise((resolve, reject) => {
        ...
        stream.on(
          'data',
          (
            response: protos.google.cloud.commerce.consumer.procurement.v1alpha1.Order
          ) => {
