...
function main(parent) {
  ...
  const {FeaturedContentNativeDashboardServiceClient} = require('@google-cloud/chronicle').v1;

  // Instantiates a client
  const chronicleClient = new FeaturedContentNativeDashboardServiceClient();

  async function callListFeaturedContentNativeDashboards() {
    // Construct request
    const request = {
      parent,
    };
    ...
    // Run request
    const iterable = chronicleClient.listFeaturedContentNativeDashboardsAsync(request);
    for await (const response of iterable) {
        console.log(response);
    }
  }

  callListFeaturedContentNativeDashboards();
