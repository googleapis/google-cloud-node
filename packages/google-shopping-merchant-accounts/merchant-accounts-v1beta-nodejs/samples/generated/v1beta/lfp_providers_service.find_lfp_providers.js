...
function main(parent) {
  ...
  const {LfpProvidersServiceClient} = require('@google-shopping/accounts').v1beta;

  // Instantiates a client
  const accountsClient = new LfpProvidersServiceClient();

  async function callFindLfpProviders() {
    // Construct request
    const request = {
      parent,
    };
    ...
    // Run request
    const iterable = accountsClient.findLfpProvidersAsync(request);
    for await (const response of iterable) {
        console.log(response);
    }
  }

  callFindLfpProviders();
