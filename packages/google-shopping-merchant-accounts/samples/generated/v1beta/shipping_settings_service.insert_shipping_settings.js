...
function main(parent, shippingSetting) {
  ...
  const {ShippingSettingsServiceClient} = require('@google-shopping/accounts').v1beta;

  // Instantiates a client
  const accountsClient = new ShippingSettingsServiceClient();

  async function callInsertShippingSettings() {
    // Construct request
    const request = {
      parent,
      shippingSetting,
    ...
    // Run request
    const response = await accountsClient.insertShippingSettings(request);
    console.log(response);
  }

  callInsertShippingSettings();
