...
function main(parent) {
  // [START networksecurity_v1_generated_Mirroring_ListMirroringEndpointGroupAssociations_async]
  /**
  ...
  const {MirroringClient} = require('@google-cloud/network-security').v1;

  // Instantiates a client
  const networksecurityClient = new MirroringClient();

  async function callListMirroringEndpointGroupAssociations() {
    // Construct request
    const request = {
      parent,
    };
    ...
    // Run request
    const iterable = networksecurityClient.listMirroringEndpointGroupAssociationsAsync(request);
    for await (const response of iterable) {
        console.log(response);
    }
  }

  callListMirroringEndpointGroupAssociations();
