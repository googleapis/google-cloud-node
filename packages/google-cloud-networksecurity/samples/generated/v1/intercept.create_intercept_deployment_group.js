...
function main(parent, interceptDeploymentGroupId, interceptDeploymentGroup) {
   ...
   */
  // const interceptDeploymentGroupId = 'abc123'
  /**
   *  Required. The deployment group to create.
   */
  // const interceptDeploymentGroup = {}
  /**
  ...
  const {InterceptClient} = require('@google-cloud/network-security').v1;

  // Instantiates a client
  const networksecurityClient = new InterceptClient();

  async function callCreateInterceptDeploymentGroup() {
    // Construct request
    const request = {
      parent,
    ...
    // Run request
    const [operation] = await networksecurityClient.createInterceptDeploymentGroup(request);
    const [response] = await operation.promise();
    console.log(response);
  }

  callCreateInterceptDeploymentGroup();
