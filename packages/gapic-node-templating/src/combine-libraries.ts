...
// --- Main Script Logic ---
export async function combineLibraries(packageDir: string, apiId: string) {
    console.log(`Starting creation of combined library: ${COMBINED_LIB_NAME}`);
    console.log(`Source libraries directory: ${BASE_LIBRARIES_DIR}`);
    console.log("---------------------------------------------------");

    try {
        // 1. Clean up existing combined directory if it exists
        try {
            await fs.rm(COMBINED_LIB_PATH, { recursive: true, force: true });
            console.log(`Removed existing combined library directory: ${COMBINED_LIB_PATH}`);
        } catch (error) {
        ...
        await fs.mkdir(COMBINED_LIB_PATH, { recursive: true });
        console.log(`Created new combined library directory: ${COMBINED_LIB_PATH}`);
        console.log("");
        ...
        console.error("An error occurred during the combination process:", error);
