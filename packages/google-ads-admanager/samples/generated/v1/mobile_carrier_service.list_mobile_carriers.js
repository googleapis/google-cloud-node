...
function main(parent) {
  // [START admanager_v1_generated_MobileCarrierService_ListMobileCarriers_async]
  /**
  ...
  const {MobileCarrierServiceClient} = require('@google-ads/admanager').v1;

  // Instantiates a client
  const admanagerClient = new MobileCarrierServiceClient();

  async function callListMobileCarriers() {
    // Construct request
    const request = {
      parent,
    };
    ...
    // Run request
    const iterable = admanagerClient.listMobileCarriersAsync(request);
    for await (const response of iterable) {
        console.log(response);
    }
  }

  callListMobileCarriers();
  // [END admanager_v1_generated_MobileCarrierService_ListMobileCarriers_async]
}
