...
function main(parent) {
  ...
  const {ContentLabelServiceClient} = require('@google-ads/admanager').v1;

  // Instantiates a client
  const admanagerClient = new ContentLabelServiceClient();

  async function callListContentLabels() {
    // Construct request
    const request = {
      parent,
    };
    ...
    // Run request
    const iterable = admanagerClient.listContentLabelsAsync(request);
    for await (const response of iterable) {
        console.log(response);
    }
  }

  callListContentLabels();
