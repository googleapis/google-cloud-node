...
function main(parent) {
  ...
  const {MobileDeviceServiceClient} = require('@google-ads/admanager').v1;

  // Instantiates a client
  const admanagerClient = new MobileDeviceServiceClient();

  async function callListMobileDevices() {
    // Construct request
    const request = {
      parent,
    };
    ...
    // Run request
    const iterable = admanagerClient.listMobileDevicesAsync(request);
    for await (const response of iterable) {
        console.log(response);
    }
  }

  callListMobileDevices();
