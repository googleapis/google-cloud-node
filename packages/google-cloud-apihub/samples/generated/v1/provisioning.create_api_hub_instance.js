...
function main(parent, apiHubInstance) {
   ...
   *  are `/[a-z][A-Z][0-9]-_/`.
   */
  // const apiHubInstanceId = 'abc123'
  /**
   *  Required. The ApiHub instance.
   */
  // const apiHubInstance = {}

  // Imports the Apihub library
  const {ProvisioningClient} = require('@google-cloud/apihub').v1;

  // Instantiates a client
  const apihubClient = new ProvisioningClient();

  async function callCreateApiHubInstance() {
    // Construct request
    const request = {
      parent,
      apiHubInstance,
    ...
    // Run request
    const [operation] = await apihubClient.createApiHubInstance(request);
    const [response] = await operation.promise();
    console.log(response);
  }

  callCreateApiHubInstance();
