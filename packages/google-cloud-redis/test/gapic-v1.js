...
describe('CloudRedisClient', () => {
  it('has servicePath', () => {
    const servicePath = redisModule.v1.CloudRedisClient.servicePath;
    assert(servicePath);
  });

  it('has apiEndpoint', () => {
    const apiEndpoint = redisModule.v1.CloudRedisClient.apiEndpoint;
    assert(apiEndpoint);
  });

  it('has port', () => {
    const port = redisModule.v1.CloudRedisClient.port;
    assert(port);
