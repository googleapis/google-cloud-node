...
function doStuffWithDomainMappingsClient(client: DomainMappingsClient) {
  client.close();
}
function doStuffWithFirewallClient(client: FirewallClient) {
  client.close();
}
...
function main() {
  ...
  doStuffWithApplicationsClient(applicationsClient);
  // check that the client instance can be created
  const authorizedCertificatesClient = new AuthorizedCertificatesClient();
  doStuffWithAuthorizedCertificatesClient(authorizedCertificatesClient);
  // check that the client instance can be created
  const authorizedDomainsClient = new AuthorizedDomainsClient();
  doStuffWithAuthorizedDomainsClient(authorizedDomainsClient);
  // check that the client instance can be created
  const domainMappingsClient = new DomainMappingsClient();
  doStuffWithDomainMappingsClient(domainMappingsClient);
  // check that the client instance can be created
  const firewallClient = new FirewallClient();
  doStuffWithFirewallClient(firewallClient);
  // check that the client instance can be created
