...
describe('v2.ExecutionsClient', () => {
  describe('Common methods', () => {
    ...
    it('does not allow setting both universeDomain and universe_domain', () => {
      assert.throws(() => {
        new executionsModule.v2.ExecutionsClient({
          universe_domain: 'example.com',
    ...
    it('invokes getExecution without error using callback', async () => {
      ...
      const promise = new Promise((resolve, reject) => {
        client.getExecution(
          ...
          (
            err?: Error | null,
            result?: protos.google.cloud.run.v2.IExecution | null
          ) => {
            if (err) {
              reject(err);
            } else {
  ...
  describe('cancelExecution', () => {
    ...
    it('invokes cancelExecution with call error', async () => {
      ...
      const expectedHeaderRequestParams = `name=${defaultValue1}`;
      const expectedError = new Error('expected');
      client.innerApiCalls.cancelExecution = stubLongRunningCall(
        undefined,
        expectedError
      );
    ...
    it('invokes checkCancelExecutionProgress with error', async () => {
      ...
      await assert.rejects(
        client.checkCancelExecutionProgress(''),
        expectedError
      );
      assert((client.operationsClient.getOperation as SinonStub).getCall(0));
    });
