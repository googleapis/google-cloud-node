...
function main(name) {
  ...
  const {OnlineEvaluatorServiceClient} = require('@google-cloud/aiplatform').v1beta1;

  // Instantiates a client
  const aiplatformClient = new OnlineEvaluatorServiceClient();

  async function callGetOnlineEvaluator() {
    // Construct request
    const request = {
      name,
    };
    ...
    // Run request
    const response = await aiplatformClient.getOnlineEvaluator(request);
    console.log(response);
  }

  callGetOnlineEvaluator();
