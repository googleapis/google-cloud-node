...
function main(parent) {
  ...
  const {ReasoningEngineRuntimeRevisionServiceClient} = require('@google-cloud/aiplatform').v1beta1;

  // Instantiates a client
  const aiplatformClient = new ReasoningEngineRuntimeRevisionServiceClient();

  async function callListReasoningEngineRuntimeRevisions() {
    // Construct request
    const request = {
      parent,
    };
    ...
    // Run request
    const iterable = aiplatformClient.listReasoningEngineRuntimeRevisionsAsync(request);
    for await (const response of iterable) {
        console.log(response);
    }
  }

  callListReasoningEngineRuntimeRevisions();
