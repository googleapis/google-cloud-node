...
function main(name) {
  ...
  const {ReasoningEngineExecutionServiceClient} = require('@google-cloud/aiplatform').v1;

  // Instantiates a client
  const aiplatformClient = new ReasoningEngineExecutionServiceClient();

  async function callStreamQueryReasoningEngine() {
    // Construct request
    const request = {
      name,
    };
    ...
    // Run request
    const stream = await aiplatformClient.streamQueryReasoningEngine(request);
    stream.on('data', (response) => { console.log(response) });
    stream.on('error', (err) => { throw(err) });
    stream.on('end', () => { /* API call completed */ });
  }

  callStreamQueryReasoningEngine();
