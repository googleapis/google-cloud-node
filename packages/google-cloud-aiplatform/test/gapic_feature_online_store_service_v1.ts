    ...
    it('invokes generateFetchAccessToken without error using callback', async () => {
      ...
      const promise = new Promise((resolve, reject) => {
        client.generateFetchAccessToken(
          ...
          (
            err?: Error | null,
            result?: protos.google.cloud.aiplatform.v1.IGenerateFetchAccessTokenResponse | null,
          ) => {
            if (err) {
              reject(err);
            } else {
    ...
    it('invokes featureViewDirectWrite without error', async () => {
      ...
      const promise = new Promise((resolve, reject) => {
        stream.on(
          'data',
          (
            response: protos.google.cloud.aiplatform.v1.FeatureViewDirectWriteResponse,
          ) => {
            resolve(response);
          },
        );
