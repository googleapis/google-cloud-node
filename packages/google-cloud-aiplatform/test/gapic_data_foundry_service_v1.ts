...
describe('v1.DataFoundryServiceClient', () => {
  describe('Common methods', () => {
    ...
    if (
      ...
      it('throws DeprecationWarning if static servicePath is used', () => {
        ...
        const servicePath =
          datafoundryserviceModule.v1.DataFoundryServiceClient.servicePath;
      ...
      it('throws DeprecationWarning if static apiEndpoint is used', () => {
        const stub = sinon.stub(process, 'emitWarning');
        const apiEndpoint =
          datafoundryserviceModule.v1.DataFoundryServiceClient.apiEndpoint;
        assert.strictEqual(apiEndpoint, 'aiplatform.googleapis.com');
        assert(stub.called);
        stub.restore();
      });
    ...
    if (typeof process === 'object' && 'env' in process) {
      describe('GOOGLE_CLOUD_UNIVERSE_DOMAIN environment variable', () => {
        it('sets apiEndpoint from environment variable', () => {
          ...
          const client =
            new datafoundryserviceModule.v1.DataFoundryServiceClient();
          const servicePath = client.apiEndpoint;
          assert.strictEqual(servicePath, 'aiplatform.example.com');
          if (saved) {
  ...
  describe('generateSyntheticData', () => {
    it('invokes generateSyntheticData without error', async () => {
      ...
      client.innerApiCalls.generateSyntheticData =
        stubSimpleCall(expectedResponse);
      const [response] = await client.generateSyntheticData(request);
      assert.deepStrictEqual(response, expectedResponse);
      const actualRequest = (
        client.innerApiCalls.generateSyntheticData as SinonStub
      ).getCall(0).args[0];
    ...
    it('invokes generateSyntheticData without error using callback', async () => {
      ...
      const promise = new Promise((resolve, reject) => {
        client.generateSyntheticData(
          ...
          (
            err?: Error | null,
            result?: protos.google.cloud.aiplatform.v1.IGenerateSyntheticDataResponse | null,
          ) => {
            if (err) {
              reject(err);
            } else {
    ...
    it('invokes generateSyntheticData with error', async () => {
      ...
      client.innerApiCalls.generateSyntheticData = stubSimpleCall(
        undefined,
        expectedError,
      );
      await assert.rejects(
        client.generateSyntheticData(request),
        expectedError,
      );
      const actualRequest = (
        client.innerApiCalls.generateSyntheticData as SinonStub
