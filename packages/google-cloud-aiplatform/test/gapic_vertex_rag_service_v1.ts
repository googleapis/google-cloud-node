...
describe('v1.VertexRagServiceClient', () => {
  ...
  describe('askContexts', () => {
    it('invokes askContexts without error', async () => {
      ...
      await client.initialize();
      const request = generateSampleMessage(
        new protos.google.cloud.aiplatform.v1.AskContextsRequest(),
      );
      const defaultValue1 = getTypeDefaultValue(
    ...
    it('invokes askContexts without error using callback', async () => {
      ...
      client.innerApiCalls.askContexts =
        stubSimpleCallWithCallback(expectedResponse);
      const promise = new Promise((resolve, reject) => {
        client.askContexts(
          request,
          (
            err?: Error | null,
            result?: protos.google.cloud.aiplatform.v1.IAskContextsResponse | null,
          ) => {
            if (err) {
              reject(err);
            } else {
    ...
    it('invokes askContexts with error', async () => {
      ...
      const expectedHeaderRequestParams = `parent=${defaultValue1 ?? ''}`;
      const expectedError = new Error('expected');
      client.innerApiCalls.askContexts = stubSimpleCall(
        undefined,
        expectedError,
      );
  ...
  describe('asyncRetrieveContexts', () => {
    it('invokes asyncRetrieveContexts without error', async () => {
      ...
      await client.initialize();
      const request = generateSampleMessage(
        new protos.google.cloud.aiplatform.v1.AsyncRetrieveContextsRequest(),
      );
      const defaultValue1 = getTypeDefaultValue(
      ...
      client.innerApiCalls.asyncRetrieveContexts =
        stubLongRunningCall(expectedResponse);
      const [operation] = await client.asyncRetrieveContexts(request);
      const [response] = await operation.promise();
      assert.deepStrictEqual(response, expectedResponse);
      const actualRequest = (
        client.innerApiCalls.asyncRetrieveContexts as SinonStub
      ).getCall(0).args[0];
      assert.deepStrictEqual(actualRequest, request);
      const actualHeaderRequestParams = (
        client.innerApiCalls.asyncRetrieveContexts as SinonStub
      ...
      const promise = new Promise((resolve, reject) => {
        client.asyncRetrieveContexts(
          ...
          (
            err?: Error | null,
            result?: LROperation<
              protos.google.cloud.aiplatform.v1.IAsyncRetrieveContextsResponse,
              protos.google.cloud.aiplatform.v1.IAsyncRetrieveContextsOperationMetadata
            > | null,
          ) => {
            if (err) {
              reject(err);
    ...
    it('invokes asyncRetrieveContexts with call error', async () => {
      ...
      const expectedHeaderRequestParams = `parent=${defaultValue1 ?? ''}`;
      const expectedError = new Error('expected');
      client.innerApiCalls.asyncRetrieveContexts = stubLongRunningCall(
        undefined,
        expectedError,
      );
      await assert.rejects(
        client.asyncRetrieveContexts(request),
        expectedError,
      );
      const actualRequest = (
        client.innerApiCalls.asyncRetrieveContexts as SinonStub
    ...
    it('invokes checkAsyncRetrieveContextsProgress without error', async () => {
      ...
      const decodedOperation = await client.checkAsyncRetrieveContextsProgress(
    ...
    it('invokes checkAsyncRetrieveContextsProgress with error', async () => {
      ...
      await assert.rejects(
        client.checkAsyncRetrieveContextsProgress(''),
        expectedError,
      );
      assert((client.operationsClient.getOperation as SinonStub).getCall(0));
    });
