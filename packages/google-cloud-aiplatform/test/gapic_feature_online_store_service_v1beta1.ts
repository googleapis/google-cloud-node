...
describe('v1beta1.FeatureOnlineStoreServiceClient', () => {
  ...
  describe('generateFetchAccessToken', () => {
    it('invokes generateFetchAccessToken without error', async () => {
      ...
      client.innerApiCalls.generateFetchAccessToken =
        stubSimpleCall(expectedResponse);
      const [response] = await client.generateFetchAccessToken(request);
      assert.deepStrictEqual(response, expectedResponse);
      const actualRequest = (
        client.innerApiCalls.generateFetchAccessToken as SinonStub
      ).getCall(0).args[0];
      assert.deepStrictEqual(actualRequest, request);
      const actualHeaderRequestParams = (
        client.innerApiCalls.generateFetchAccessToken as SinonStub
    ...
    it('invokes generateFetchAccessToken without error using callback', async () => {
      ...
      const promise = new Promise((resolve, reject) => {
        client.generateFetchAccessToken(
          ...
          (
            err?: Error | null,
            result?: protos.google.cloud.aiplatform.v1beta1.IGenerateFetchAccessTokenResponse | null,
          ) => {
            if (err) {
              reject(err);
            } else {
    ...
    it('invokes generateFetchAccessToken with error', async () => {
      ...
      const expectedHeaderRequestParams = `feature_view=${defaultValue1 ?? ''}`;
      const expectedError = new Error('expected');
      client.innerApiCalls.generateFetchAccessToken = stubSimpleCall(
        undefined,
        expectedError,
      );
      await assert.rejects(
  ...
  describe('featureViewDirectWrite', () => {
    it('invokes featureViewDirectWrite without error', async () => {
      ...
      client.innerApiCalls.featureViewDirectWrite =
        stubBidiStreamingCall(expectedResponse);
      ...
      const promise = new Promise((resolve, reject) => {
        stream.on(
          'data',
          (
            response: protos.google.cloud.aiplatform.v1beta1.FeatureViewDirectWriteResponse,
          ) => {
            resolve(response);
          },
        );
      ...
      assert.deepStrictEqual(response, expectedResponse);
      assert(
        (client.innerApiCalls.featureViewDirectWrite as SinonStub)
          .getCall(0)
          .calledWith(null),
      );
      assert.deepStrictEqual(
    ...
    it('invokes featureViewDirectWrite with error', async () => {
      ...
      const expectedError = new Error('expected');
      client.innerApiCalls.featureViewDirectWrite = stubBidiStreamingCall(
        undefined,
        expectedError,
      );
      const stream = client.featureViewDirectWrite();
