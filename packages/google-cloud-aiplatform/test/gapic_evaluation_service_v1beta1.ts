...
describe('v1beta1.EvaluationServiceClient', () => {
  ...
  describe('generateInstanceRubrics', () => {
    it('invokes generateInstanceRubrics without error', async () => {
      ...
      client.innerApiCalls.generateInstanceRubrics =
        stubSimpleCall(expectedResponse);
      const [response] = await client.generateInstanceRubrics(request);
      assert.deepStrictEqual(response, expectedResponse);
      const actualRequest = (
        client.innerApiCalls.generateInstanceRubrics as SinonStub
      ).getCall(0).args[0];
      assert.deepStrictEqual(actualRequest, request);
      const actualHeaderRequestParams = (
        client.innerApiCalls.generateInstanceRubrics as SinonStub
    ...
    it('invokes generateInstanceRubrics without error using callback', async () => {
      ...
      const promise = new Promise((resolve, reject) => {
        client.generateInstanceRubrics(
          ...
          (
            err?: Error | null,
            result?: protos.google.cloud.aiplatform.v1beta1.IGenerateInstanceRubricsResponse | null,
          ) => {
            if (err) {
              reject(err);
            } else {
    ...
    it('invokes generateInstanceRubrics with error', async () => {
      ...
      const expectedHeaderRequestParams = `location=${defaultValue1 ?? ''}`;
      const expectedError = new Error('expected');
      client.innerApiCalls.generateInstanceRubrics = stubSimpleCall(
        undefined,
        expectedError,
      );
  ...
  describe('evaluateDataset', () => {
    it('invokes evaluateDataset without error', async () => {
      ...
      client.innerApiCalls.evaluateDataset =
        stubLongRunningCall(expectedResponse);
      const [operation] = await client.evaluateDataset(request);
      const [response] = await operation.promise();
      assert.deepStrictEqual(response, expectedResponse);
      const actualRequest = (
        client.innerApiCalls.evaluateDataset as SinonStub
      ).getCall(0).args[0];
      assert.deepStrictEqual(actualRequest, request);
      const actualHeaderRequestParams = (
        client.innerApiCalls.evaluateDataset as SinonStub
      ...
      const promise = new Promise((resolve, reject) => {
        client.evaluateDataset(
          ...
          (
            err?: Error | null,
            result?: LROperation<
              protos.google.cloud.aiplatform.v1beta1.IEvaluateDatasetResponse,
              protos.google.cloud.aiplatform.v1beta1.IEvaluateDatasetOperationMetadata
            > | null,
          ) => {
            if (err) {
              reject(err);
    ...
    it('invokes evaluateDataset with call error', async () => {
      ...
      const expectedHeaderRequestParams = `location=${defaultValue1 ?? ''}`;
      const expectedError = new Error('expected');
      client.innerApiCalls.evaluateDataset = stubLongRunningCall(
        undefined,
        expectedError,
      );
    ...
    it('invokes checkEvaluateDatasetProgress without error', async () => {
      ...
      const decodedOperation = await client.checkEvaluateDatasetProgress(
    ...
    it('invokes checkEvaluateDatasetProgress with error', async () => {
      ...
      await assert.rejects(
        client.checkEvaluateDatasetProgress(''),
        expectedError,
      );
      assert((client.operationsClient.getOperation as SinonStub).getCall(0));
    });
