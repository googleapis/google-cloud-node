...
function main(name) {
  // [START discoveryengine_v1_generated_SiteSearchEngineService_DeleteSitemap_async]
  /**
  ...
  const {SiteSearchEngineServiceClient} = require('@google-cloud/discoveryengine').v1;

  // Instantiates a client
  const discoveryengineClient = new SiteSearchEngineServiceClient();

  async function callDeleteSitemap() {
    // Construct request
    const request = {
      name,
    };
    ...
    // Run request
    const [operation] = await discoveryengineClient.deleteSitemap(request);
    const [response] = await operation.promise();
    console.log(response);
  }

  callDeleteSitemap();
  // [END discoveryengine_v1_generated_SiteSearchEngineService_DeleteSitemap_async]
}
