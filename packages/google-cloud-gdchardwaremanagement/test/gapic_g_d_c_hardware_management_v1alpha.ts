...
describe('v1alpha.GDCHardwareManagementClient', () => {
  ...
  describe('requestOrderDateChange', () => {
    it('invokes requestOrderDateChange without error', async () => {
      ...
      client.innerApiCalls.requestOrderDateChange =
        stubLongRunningCall(expectedResponse);
      const [operation] = await client.requestOrderDateChange(request);
      const [response] = await operation.promise();
      assert.deepStrictEqual(response, expectedResponse);
      const actualRequest = (
        client.innerApiCalls.requestOrderDateChange as SinonStub
      ).getCall(0).args[0];
      assert.deepStrictEqual(actualRequest, request);
      const actualHeaderRequestParams = (
        client.innerApiCalls.requestOrderDateChange as SinonStub
    ...
    it('invokes requestOrderDateChange with call error', async () => {
      ...
      const expectedHeaderRequestParams = `name=${defaultValue1 ?? ''}`;
      const expectedError = new Error('expected');
      client.innerApiCalls.requestOrderDateChange = stubLongRunningCall(
        undefined,
        expectedError,
      );
      await assert.rejects(
        client.requestOrderDateChange(request),
        expectedError,
      );
      const actualRequest = (
        client.innerApiCalls.requestOrderDateChange as SinonStub
    ...
    it('invokes checkRequestOrderDateChangeProgress with error', async () => {
      ...
      await assert.rejects(
        client.checkRequestOrderDateChangeProgress(''),
        expectedError,
      );
      assert((client.operationsClient.getOperation as SinonStub).getCall(0));
    });
