...
export class PagesClient {
  ...
  constructor(opts?: ClientOptions) {
    ...
    // pass `{fallback: true}` to the PagesClient constructor.
    this._gaxModule = opts.fallback ? gax.fallback : gax;

    // Create a `gaxGrpc` object, with any grpc-specific options
    // sent to the client.
    opts.scopes = (this.constructor as typeof PagesClient).scopes;
   ...
   *   The first element is Array of [Page]{@link google.cloud.dialogflow.cx.v3beta1.Page} that corresponds to
   *   the one page received from the API server.
   *   If the second element is not null it contains the request object of type [ListPagesRequest]{@link google.cloud.dialogflow.cx.v3beta1.ListPagesRequest}
   *   that can be used to obtain the next page of the results.
   *   If it is null, the next page does not exist.
   *   The third element contains the raw response received from the API server. Its type is
   *   [ListPagesResponse]{@link google.cloud.dialogflow.cx.v3beta1.ListPagesResponse}.
   *
   *   The promise has a method named "cancel" which cancels the ongoing API call.
   */
  listPages(
  ...
  /**
   * Equivalent to {@link listPages}, but returns a NodeJS Stream object.
   *
   * This fetches the paged responses for {@link listPages} continuously
  ...
  /**
   * Equivalent to {@link listPages}, but returns an iterable object.
   *
