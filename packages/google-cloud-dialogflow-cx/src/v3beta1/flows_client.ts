...
export class FlowsClient {
  ...
  constructor(opts?: ClientOptions) {
    ...
    // pass `{fallback: true}` to the FlowsClient constructor.
    this._gaxModule = opts.fallback ? gax.fallback : gax;

    // Create a `gaxGrpc` object, with any grpc-specific options
    // sent to the client.
    opts.scopes = (this.constructor as typeof FlowsClient).scopes;
  ...
  /**
   * Check the status of the long running operation returned by the trainFlow() method.
   * @param {String} name
   ...
   *   The first element is Array of [Flow]{@link google.cloud.dialogflow.cx.v3beta1.Flow} that corresponds to
   *   the one page received from the API server.
   *   If the second element is not null it contains the request object of type [ListFlowsRequest]{@link google.cloud.dialogflow.cx.v3beta1.ListFlowsRequest}
   *   that can be used to obtain the next page of the results.
   *   If it is null, the next page does not exist.
   *   The third element contains the raw response received from the API server. Its type is
   *   [ListFlowsResponse]{@link google.cloud.dialogflow.cx.v3beta1.ListFlowsResponse}.
   *
   *   The promise has a method named "cancel" which cancels the ongoing API call.
   */
  listFlows(
  ...
  /**
   * Equivalent to {@link listFlows}, but returns a NodeJS Stream object.
   *
   * This fetches the paged responses for {@link listFlows} continuously
  ...
  /**
   * Equivalent to {@link listFlows}, but returns an iterable object.
   *
