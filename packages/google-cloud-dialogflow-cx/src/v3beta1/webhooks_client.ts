...
export class WebhooksClient {
  ...
  constructor(opts?: ClientOptions) {
    ...
    // pass `{fallback: true}` to the WebhooksClient constructor.
    this._gaxModule = opts.fallback ? gax.fallback : gax;

    // Create a `gaxGrpc` object, with any grpc-specific options
    // sent to the client.
    opts.scopes = (this.constructor as typeof WebhooksClient).scopes;
   ...
   *   The first element is Array of [Webhook]{@link google.cloud.dialogflow.cx.v3beta1.Webhook} that corresponds to
   *   the one page received from the API server.
   *   If the second element is not null it contains the request object of type [ListWebhooksRequest]{@link google.cloud.dialogflow.cx.v3beta1.ListWebhooksRequest}
   *   that can be used to obtain the next page of the results.
   *   If it is null, the next page does not exist.
   *   The third element contains the raw response received from the API server. Its type is
   *   [ListWebhooksResponse]{@link google.cloud.dialogflow.cx.v3beta1.ListWebhooksResponse}.
   *
   *   The promise has a method named "cancel" which cancels the ongoing API call.
   */
  listWebhooks(
  ...
  /**
   * Equivalent to {@link listWebhooks}, but returns a NodeJS Stream object.
   *
   * This fetches the paged responses for {@link listWebhooks} continuously
  ...
  /**
   * Equivalent to {@link listWebhooks}, but returns an iterable object.
   *
