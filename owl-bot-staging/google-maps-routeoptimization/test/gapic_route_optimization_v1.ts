...
describe('v1.RouteOptimizationClient', () => {
  describe('Common methods', () => {
    ...
    if (
      ...
      it('throws DeprecationWarning if static servicePath is used', () => {
        ...
        const servicePath =
          routeoptimizationModule.v1.RouteOptimizationClient.servicePath;
      ...
      it('throws DeprecationWarning if static apiEndpoint is used', () => {
        const stub = sinon.stub(process, 'emitWarning');
        const apiEndpoint =
          routeoptimizationModule.v1.RouteOptimizationClient.apiEndpoint;
        assert.strictEqual(apiEndpoint, 'routeoptimization.googleapis.com');
        assert(stub.called);
        stub.restore();
      });
    ...
    if (typeof process === 'object' && 'env' in process) {
      describe('GOOGLE_CLOUD_UNIVERSE_DOMAIN environment variable', () => {
        it('sets apiEndpoint from environment variable', () => {
          ...
          const client =
            new routeoptimizationModule.v1.RouteOptimizationClient();
          const servicePath = client.apiEndpoint;
          assert.strictEqual(servicePath, 'routeoptimization.example.com');
          if (saved) {
        ...
        it('value configured in code has priority over environment variable', () => {
          ...
          assert.strictEqual(
            servicePath,
            'routeoptimization.configured.example.com',
          );
          if (saved) {
            process.env['GOOGLE_CLOUD_UNIVERSE_DOMAIN'] = saved;
          } else {
    ...
    it('invokes optimizeTours without error using callback', async () => {
      ...
      const promise = new Promise((resolve, reject) => {
        client.optimizeTours(
          ...
          (
            err?: Error | null,
            result?: protos.google.maps.routeoptimization.v1.IOptimizeToursResponse | null,
          ) => {
            if (err) {
              reject(err);
            } else {
  ...
  describe('optimizeToursLongRunning', () => {
    it('invokes optimizeToursLongRunning without error', async () => {
      ...
      client.innerApiCalls.optimizeToursLongRunning =
        stubLongRunningCall(expectedResponse);
      const [operation] = await client.optimizeToursLongRunning(request);
      const [response] = await operation.promise();
      assert.deepStrictEqual(response, expectedResponse);
      const actualRequest = (
        client.innerApiCalls.optimizeToursLongRunning as SinonStub
      ).getCall(0).args[0];
      assert.deepStrictEqual(actualRequest, request);
      const actualHeaderRequestParams = (
        client.innerApiCalls.optimizeToursLongRunning as SinonStub
    ...
    it('invokes optimizeToursLongRunning with call error', async () => {
      ...
      const expectedHeaderRequestParams = `parent=${defaultValue1 ?? ''}`;
      const expectedError = new Error('expected');
      client.innerApiCalls.optimizeToursLongRunning = stubLongRunningCall(
        undefined,
        expectedError,
      );
      await assert.rejects(
        client.optimizeToursLongRunning(request),
        expectedError,
      );
      const actualRequest = (
        client.innerApiCalls.optimizeToursLongRunning as SinonStub
    ...
    it('invokes checkOptimizeToursLongRunningProgress without error', async () => {
      ...
      const decodedOperation =
        await client.checkOptimizeToursLongRunningProgress(
          expectedResponse.name,
        );
      assert.deepStrictEqual(decodedOperation.name, expectedResponse.name);
      assert(decodedOperation.metadata);
    ...
    it('invokes checkOptimizeToursLongRunningProgress with error', async () => {
      ...
      await assert.rejects(
        client.checkOptimizeToursLongRunningProgress(''),
        expectedError,
      );
      assert((client.operationsClient.getOperation as SinonStub).getCall(0));
    });
  ...
  describe('optimizeToursUri', () => {
    it('invokes optimizeToursUri without error', async () => {
      ...
      client.innerApiCalls.optimizeToursUri =
        stubLongRunningCall(expectedResponse);
      const [operation] = await client.optimizeToursUri(request);
      const [response] = await operation.promise();
      assert.deepStrictEqual(response, expectedResponse);
      const actualRequest = (
        client.innerApiCalls.optimizeToursUri as SinonStub
      ).getCall(0).args[0];
      assert.deepStrictEqual(actualRequest, request);
      const actualHeaderRequestParams = (
        client.innerApiCalls.optimizeToursUri as SinonStub
    ...
    it('invokes optimizeToursUri with call error', async () => {
      ...
      const expectedHeaderRequestParams = `parent=${defaultValue1 ?? ''}`;
      const expectedError = new Error('expected');
      client.innerApiCalls.optimizeToursUri = stubLongRunningCall(
        undefined,
        expectedError,
      );
