...
function main(name) {
  ...
  const {BusinessIdentityServiceClient} = require('@google-shopping/accounts').v1beta;

  // Instantiates a client
  const accountsClient = new BusinessIdentityServiceClient();

  async function callGetBusinessIdentity() {
    // Construct request
    const request = {
      name,
    };
    ...
    // Run request
    const response = await accountsClient.getBusinessIdentity(request);
    console.log(response);
  }

  callGetBusinessIdentity();
