  ...
  afterEach(() => {
        ...
        if (typeof process === 'object' && 'env' in process) {
            describe('GOOGLE_CLOUD_UNIVERSE_DOMAIN environment variable', () => {
                ...
                it('value configured in code has priority over environment variable', () => {
                    const saved = process.env['GOOGLE_CLOUD_UNIVERSE_DOMAIN'];
                    process.env['GOOGLE_CLOUD_UNIVERSE_DOMAIN'] = 'example.com';
                    const client = new addressesModule.v1.AddressesClient({universeDomain: 'configured.example.com'});
                    const servicePath = client.apiEndpoint;
    ...
    describe('move', () => {
        it('invokes move without error', async () => {
            ...
            await client.initialize();
            const request = generateSampleMessage(
              new protos.google.cloud.compute.v1.MoveAddressRequest()
            );
            const defaultValue1 =
              getTypeDefaultValue('.google.cloud.compute.v1.MoveAddressRequest', ['project']);
            request.project = defaultValue1;
            const defaultValue2 =
              getTypeDefaultValue('.google.cloud.compute.v1.MoveAddressRequest', ['region']);
            request.region = defaultValue2;
            const defaultValue3 =
              getTypeDefaultValue('.google.cloud.compute.v1.MoveAddressRequest', ['address']);
            request.address = defaultValue3;
            const expectedHeaderRequestParams = `project=${defaultValue1 ?? '' }&region=${defaultValue2 ?? '' }&address=${defaultValue3 ?? '' }`;
            const expectedResponse = generateSampleMessage(
              new protos.google.cloud.compute.v1.Operation()
