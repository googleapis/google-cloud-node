...
describe('v1beta.QuotaAdjusterSettingsManagerClient', () => {
    ...
    describe('Path templates', () => {
        ...
        describe('quotaAdjusterSettings', () => {
            ...
            client.initialize();
            client.pathTemplates.quotaAdjusterSettingsPathTemplate.render =
                sinon.stub().returns(fakePath);
            client.pathTemplates.quotaAdjusterSettingsPathTemplate.match =
                sinon.stub().returns(expectedParameters);

            it('quotaAdjusterSettingsPath', () => {
                const result = client.quotaAdjusterSettingsPath("projectValue", "locationValue");
                assert.strictEqual(result, fakePath);
                assert((client.pathTemplates.quotaAdjusterSettingsPathTemplate.render as SinonStub)
                    .getCall(-1).calledWith(expectedParameters));
            });

            it('matchProjectFromQuotaAdjusterSettingsName', () => {
                const result = client.matchProjectFromQuotaAdjusterSettingsName(fakePath);
                assert.strictEqual(result, "projectValue");
                assert((client.pathTemplates.quotaAdjusterSettingsPathTemplate.match as SinonStub)
                    .getCall(-1).calledWith(fakePath));
            });

            it('matchLocationFromQuotaAdjusterSettingsName', () => {
                const result = client.matchLocationFromQuotaAdjusterSettingsName(fakePath);
                assert.strictEqual(result, "locationValue");
                assert((client.pathTemplates.quotaAdjusterSettingsPathTemplate.match as SinonStub)
                    .getCall(-1).calledWith(fakePath));
            });
