...
describe('v2.ProductServiceClient', () => {
    describe('Common methods', () => {
        ...
        it('sets apiEndpoint according to universe domain camelCase', () => {
            const client = new productserviceModule.v2.ProductServiceClient({universeDomain: 'example.com'});
            const servicePath = client.apiEndpoint;
            assert.strictEqual(servicePath, 'retail.example.com');
        });

        it('sets apiEndpoint according to universe domain snakeCase', () => {
            const client = new productserviceModule.v2.ProductServiceClient({universe_domain: 'example.com'});
            const servicePath = client.apiEndpoint;
            assert.strictEqual(servicePath, 'retail.example.com');
        });
        ...
        if (typeof process === 'object' && 'env' in process) {
            describe('GOOGLE_CLOUD_UNIVERSE_DOMAIN environment variable', () => {
                ...
                it('value configured in code has priority over environment variable', () => {
                    const saved = process.env['GOOGLE_CLOUD_UNIVERSE_DOMAIN'];
                    process.env['GOOGLE_CLOUD_UNIVERSE_DOMAIN'] = 'example.com';
                    const client = new productserviceModule.v2.ProductServiceClient({universeDomain: 'configured.example.com'});
                    const servicePath = client.apiEndpoint;
        ...
        it('does not allow setting both universeDomain and universe_domain', () => {
            assert.throws(() => { new productserviceModule.v2.ProductServiceClient({universe_domain: 'example.com', universeDomain: 'example.net'}); });
        });
    ...
    describe('purgeProducts', () => {
        it('invokes purgeProducts without error', async () => {
            ...
            await client.initialize();
            const request = generateSampleMessage(
              new protos.google.cloud.retail.v2.PurgeProductsRequest()
            );
            const defaultValue1 =
              getTypeDefaultValue('.google.cloud.retail.v2.PurgeProductsRequest', ['parent']);
            request.parent = defaultValue1;
            const expectedHeaderRequestParams = `parent=${defaultValue1 ?? '' }`;
            const expectedResponse = generateSampleMessage(
        ...
        it('invokes purgeProducts without error using callback', async () => {
            ...
            const promise = new Promise((resolve, reject) => {
                 client.purgeProducts(
                    request,
                    (err?: Error|null,
                     result?: LROperation<protos.google.cloud.retail.v2.IPurgeProductsResponse, protos.google.cloud.retail.v2.IPurgeProductsMetadata>|null
                    ) => {
                        if (err) {
                            reject(err);
                        } else {
            ...
            const operation = await promise as LROperation<protos.google.cloud.retail.v2.IPurgeProductsResponse, protos.google.cloud.retail.v2.IPurgeProductsMetadata>;
            const [response] = await operation.promise();
            assert.deepStrictEqual(response, expectedResponse);
            const actualRequest = (client.innerApiCalls.purgeProducts as SinonStub)
                .getCall(0).args[0];
