...
describe('v1.DataTransferServiceClient', () => {
        ...
        if (typeof process === 'object' && 'env' in process) {
            describe('GOOGLE_CLOUD_UNIVERSE_DOMAIN environment variable', () => {
                ...
                it('value configured in code has priority over environment variable', () => {
                    ...
                    const servicePath = client.apiEndpoint;
                    assert.strictEqual(servicePath, 'bigquerydatatransfer.configured.example.com');
                    if (saved) {
                        process.env['GOOGLE_CLOUD_UNIVERSE_DOMAIN'] = saved;
                    } else {
                        delete process.env['GOOGLE_CLOUD_UNIVERSE_DOMAIN'];
    ...
    describe('unenrollDataSources', () => {
        it('invokes unenrollDataSources without error', async () => {
            ...
            const defaultValue1 =
              getTypeDefaultValue('.google.cloud.bigquery.datatransfer.v1.UnenrollDataSourcesRequest', ['name']);
            request.name = defaultValue1;
            const expectedHeaderRequestParams = `name=${defaultValue1 ?? '' }`;
            const expectedResponse = generateSampleMessage(
              new protos.google.protobuf.Empty()
            );
            client.innerApiCalls.unenrollDataSources = stubSimpleCall(expectedResponse);
            const [response] = await client.unenrollDataSources(request);
            assert.deepStrictEqual(response, expectedResponse);
            const actualRequest = (client.innerApiCalls.unenrollDataSources as SinonStub)
                .getCall(0).args[0];
            assert.deepStrictEqual(actualRequest, request);
            const actualHeaderRequestParams = (client.innerApiCalls.unenrollDataSources as SinonStub)
                .getCall(0).args[1].otherArgs.headers['x-goog-request-params'];
            assert(actualHeaderRequestParams.includes(expectedHeaderRequestParams));
        });
        ...
        it('invokes unenrollDataSources without error using callback', async () => {
            ...
            client.innerApiCalls.unenrollDataSources = stubSimpleCallWithCallback(expectedResponse);
            const promise = new Promise((resolve, reject) => {
        ...
        it('invokes unenrollDataSources with error', async () => {
            ...
            const expectedHeaderRequestParams = `name=${defaultValue1 ?? '' }`;
            const expectedError = new Error('expected');
            client.innerApiCalls.unenrollDataSources = stubSimpleCall(undefined, expectedError);
