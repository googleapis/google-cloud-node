...
function main(parent) {
  // [START admanager_v1_generated_BrowserLanguageService_ListBrowserLanguages_async]
  /**
  ...
  const {BrowserLanguageServiceClient} = require('@google-ads/admanager').v1;

  // Instantiates a client
  const admanagerClient = new BrowserLanguageServiceClient();

  async function callListBrowserLanguages() {
    // Construct request
    const request = {
      parent,
    };
    ...
    // Run request
    const iterable = admanagerClient.listBrowserLanguagesAsync(request);
    for await (const response of iterable) {
        console.log(response);
    }
  }

  callListBrowserLanguages();
