 ...
 * `src/v1/partition_assignment_service_client_config.json`.
 * This file defines retry strategy and timeouts for all API methods in this library.
 */
import * as gapicConfig from './partition_assignment_service_client_config.json';
 ...
 * @memberof v1
 */
export class PartitionAssignmentServiceClient {
  ...
  partitionAssignmentServiceStub?: Promise<{[name: string]: Function}>;

  /**
   * Construct an instance of PartitionAssignmentServiceClient.
   *
   ...
   *     const client = new PartitionAssignmentServiceClient({fallback: 'rest'}, gax);
   *     ```
   */
  constructor(opts?: ClientOptions, gaxInstance?: typeof gax | typeof gax.fallback) {
    // Ensure that options include all the required fields.
    const staticMembers = this.constructor as typeof PartitionAssignmentServiceClient;
    ...
    this.descriptors.stream = {
      assignPartitions: new this._gaxModule.StreamDescriptor(this._gaxModule.StreamType.BIDI_STREAMING, opts.fallback === 'rest')
    };

    // Put together the default options sent with requests.
    this._defaults = this._gaxGrpc.constructSettings(
        'google.cloud.pubsublite.v1.PartitionAssignmentService', gapicConfig as gax.ClientConfig,
  ...
  initialize() {
    ...
    if (this.partitionAssignmentServiceStub) {
      return this.partitionAssignmentServiceStub;
    }

    // Put together the "service stub" for
    // google.cloud.pubsublite.v1.PartitionAssignmentService.
    this.partitionAssignmentServiceStub = this._gaxGrpc.createStub(
        this._opts.fallback ?
          (this._protos as protobuf.Root).lookupService('google.cloud.pubsublite.v1.PartitionAssignmentService') :
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          (this._protos as any).google.cloud.pubsublite.v1.PartitionAssignmentService,
        this._opts, this._providedCustomServicePath) as Promise<{[method: string]: Function}>;

    // Iterate over each of the methods that the service provides
    // and create an API call method for each.
    const partitionAssignmentServiceStubMethods =
        ['assignPartitions'];
    for (const methodName of partitionAssignmentServiceStubMethods) {
      const callPromise = this.partitionAssignmentServiceStub.then(
        stub => (...args: Array<{}>) => {
          if (this._terminated) {
 ...
 *   representing [PartitionAssignmentRequest]{@link google.cloud.pubsublite.v1.PartitionAssignmentRequest} for write() method, and
 *   will emit objects representing [PartitionAssignment]{@link google.cloud.pubsublite.v1.PartitionAssignment} on 'data' event asynchronously.
 *   Please see the
 ...
 */
  assignPartitions(
    ...
    this.initialize();
    return this.innerApiCalls.assignPartitions(null, options);
  }
   ...
   */
  close(): Promise<void> {
    if (this.partitionAssignmentServiceStub && !this._terminated) {
      return this.partitionAssignmentServiceStub.then(stub => {
