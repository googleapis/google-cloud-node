...
describe('v1.SessionServiceClient', () => {
    ...
    describe('runSession', () => {
        ...
        it('invokes runSession without error using callback', async () => {
            ...
            const promise = new Promise((resolve, reject) => {
                 client.runSession(
                    request,
                    (err?: Error|null, result?: protos.google.cloud.ces.v1.IRunSessionResponse|null) => {
                        if (err) {
                            reject(err);
                        } else {
                            resolve(result);
    ...
    describe('bidiRunSession', () => {
        it('invokes bidiRunSession without error', async () => {
            ...
            await client.initialize();
            const request = generateSampleMessage(
              new protos.google.cloud.ces.v1.BidiSessionClientMessage()
            );
            
            const expectedResponse = generateSampleMessage(
              new protos.google.cloud.ces.v1.BidiSessionServerMessage()
            );
            client.innerApiCalls.bidiRunSession = stubBidiStreamingCall(expectedResponse);
            const stream = client.bidiRunSession();
            const promise = new Promise((resolve, reject) => {
                stream.on('data', (response: protos.google.cloud.ces.v1.BidiSessionServerMessage) => {
                    resolve(response);
                });
                stream.on('error', (err: Error) => {
                    reject(err);
