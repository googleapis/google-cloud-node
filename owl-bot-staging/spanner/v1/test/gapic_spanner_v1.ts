...
describe('v1.SpannerClient', () => {
    ...
    describe('fetchCacheUpdate', () => {
        it('invokes fetchCacheUpdate without error', async () => {
            ...
            const defaultValue1 =
              getTypeDefaultValue('.google.spanner.v1.FetchCacheUpdateRequest', ['database']);
            request.database = defaultValue1;
            const expectedHeaderRequestParams = `database=${defaultValue1 ?? '' }`;
            const expectedResponse = generateSampleMessage(
              new protos.google.spanner.v1.CacheUpdate()
            );
            client.innerApiCalls.fetchCacheUpdate = stubServerStreamingCall(expectedResponse);
            ...
            const response = await promise;
            assert.deepStrictEqual(response, expectedResponse);
            const actualRequest = (client.innerApiCalls.fetchCacheUpdate as SinonStub)
                .getCall(0).args[0];
            assert.deepStrictEqual(actualRequest, request);
            const actualHeaderRequestParams = (client.innerApiCalls.fetchCacheUpdate as SinonStub)
                .getCall(0).args[1].otherArgs.headers['x-goog-request-params'];
            assert(actualHeaderRequestParams.includes(expectedHeaderRequestParams));
        });
        ...
        it('invokes fetchCacheUpdate with closed client', async () => {
            ...
            const expectedError = new Error('The client has already been closed.');
            client.close().catch(err => {throw err});
            const stream = client.fetchCacheUpdate(request, {retryRequestOptions: {noResponseRetries: 0}});
            const promise = new Promise((resolve, reject) => {
                stream.on('data', (response: protos.google.spanner.v1.CacheUpdate) => {
                    resolve(response);
                });
