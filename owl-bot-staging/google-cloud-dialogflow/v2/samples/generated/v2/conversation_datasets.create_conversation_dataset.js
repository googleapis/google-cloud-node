...
function main(parent, conversationDataset) {
  ...
  const {ConversationDatasetsClient} = require('@google-cloud/dialogflow').v2;

  // Instantiates a client
  const dialogflowClient = new ConversationDatasetsClient();

  async function callCreateConversationDataset() {
    // Construct request
    const request = {
      parent,
      conversationDataset,
    ...
    // Run request
    const [operation] = await dialogflowClient.createConversationDataset(request);
    const [response] = await operation.promise();
    console.log(response);
  }

  callCreateConversationDataset();
