...
describe('v2.CloudTasksClient', () => {
    ...
    describe('getQueue', () => {
        ...
        it('invokes getQueue without error using callback', async () => {
            ...
            const promise = new Promise((resolve, reject) => {
                 client.getQueue(
                    request,
                    (err?: Error|null, result?: protos.google.cloud.tasks.v2.IQueue|null) => {
                        if (err) {
                            reject(err);
                        } else {
                            resolve(result);
    ...
    describe('getTask', () => {
        ...
        it('invokes getTask without error using callback', async () => {
            ...
            const promise = new Promise((resolve, reject) => {
                 client.getTask(
                    request,
                    (err?: Error|null, result?: protos.google.cloud.tasks.v2.ITask|null) => {
                        if (err) {
                            reject(err);
                        } else {
                            resolve(result);
    ...
    describe('Path templates', () => {
        ...
        describe('queue', () => {
            ...
            client.initialize();
            client.pathTemplates.queuePathTemplate.render =
                sinon.stub().returns(fakePath);
            client.pathTemplates.queuePathTemplate.match =
                sinon.stub().returns(expectedParameters);

            it('queuePath', () => {
                const result = client.queuePath("projectValue", "locationValue", "queueValue");
                assert.strictEqual(result, fakePath);
                assert((client.pathTemplates.queuePathTemplate.render as SinonStub)
                    .getCall(-1).calledWith(expectedParameters));
            });
            ...
            it('matchProjectFromQueueName', () => {
                ...
                assert.strictEqual(result, "projectValue");
                assert((client.pathTemplates.queuePathTemplate.match as SinonStub)
                    .getCall(-1).calledWith(fakePath));
            });
