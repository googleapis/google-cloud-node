...
describe('v2beta2.CloudTasksClient', () => {
    ...
    describe('bufferTask', () => {
        it('invokes bufferTask without error', async () => {
            ...
            const defaultValue1 =
              getTypeDefaultValue('.google.cloud.tasks.v2beta2.BufferTaskRequest', ['queue']);
            request.queue = defaultValue1;
            const defaultValue2 =
              getTypeDefaultValue('.google.cloud.tasks.v2beta2.BufferTaskRequest', ['taskId']);
            ...
            assert.deepStrictEqual(response, expectedResponse);
            const actualRequest = (client.innerApiCalls.bufferTask as SinonStub)
                .getCall(0).args[0];
            assert.deepStrictEqual(actualRequest, request);
            const actualHeaderRequestParams = (client.innerApiCalls.bufferTask as SinonStub)
                .getCall(0).args[1].otherArgs.headers['x-goog-request-params'];
            assert(actualHeaderRequestParams.includes(expectedHeaderRequestParams));
        });
        ...
        it('invokes bufferTask without error using callback', async () => {
            ...
            client.innerApiCalls.bufferTask = stubSimpleCallWithCallback(expectedResponse);
            const promise = new Promise((resolve, reject) => {
                 client.bufferTask(
                    request,
                    (err?: Error|null, result?: protos.google.cloud.tasks.v2beta2.IBufferTaskResponse|null) => {
                        if (err) {
                            reject(err);
                        } else {
                            resolve(result);
        ...
        it('invokes bufferTask with error', async () => {
            ...
            const expectedHeaderRequestParams = `queue=${defaultValue1 ?? '' }&task_id=${defaultValue2 ?? '' }`;
            const expectedError = new Error('expected');
            client.innerApiCalls.bufferTask = stubSimpleCall(undefined, expectedError);
