...
function main(name) {
  ...
  const {PlaybooksClient} = require('@google-cloud/dialogflow-cx').v3;

  // Instantiates a client
  const cxClient = new PlaybooksClient();

  async function callExportPlaybook() {
    // Construct request
    const request = {
      name,
    };
    ...
    // Run request
    const [operation] = await cxClient.exportPlaybook(request);
    const [response] = await operation.promise();
    console.log(response);
  }

  callExportPlaybook();
  // [END dialogflow_v3_generated_Playbooks_ExportPlaybook_async]
}
