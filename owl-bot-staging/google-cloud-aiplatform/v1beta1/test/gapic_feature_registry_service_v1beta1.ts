        ...
        it('invokes createFeature without error using callback', async () => {
            ...
            const promise = new Promise((resolve, reject) => {
                 client.createFeature(
                    request,
                    (err?: Error|null,
                     result?: LROperation<protos.google.cloud.aiplatform.v1beta1.IFeature, protos.google.cloud.aiplatform.v1beta1.ICreateRegistryFeatureOperationMetadata>|null
                    ) => {
                        if (err) {
                            reject(err);
                        } else {
            ...
            const operation = await promise as LROperation<protos.google.cloud.aiplatform.v1beta1.IFeature, protos.google.cloud.aiplatform.v1beta1.ICreateRegistryFeatureOperationMetadata>;
            const [response] = await operation.promise();
            assert.deepStrictEqual(response, expectedResponse);
            const actualRequest = (client.innerApiCalls.createFeature as SinonStub)
                .getCall(0).args[0];
