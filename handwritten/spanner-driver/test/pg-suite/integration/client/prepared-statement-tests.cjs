...
;(function () {
  ...
  // Client-side named prepared statement resolution is skipped as we dont support it. 
  suite.skip('first named prepared statement', async function () {
  ...
  suite.skip('second named prepared statement with same name & text', async function () {
    const client = helper.client()
    await helper.createPersonTable(client)
    return new Promise((resolve) => {
  ...
  suite.skip('with same name, but without query text', async function () {
    const client = helper.client()
    await helper.createPersonTable(client)
  ...
  suite.skip('with same name, but with different text', async function () {
    const client = helper.client()
    await helper.createPersonTable(client)
    // First, register the named statement
    await new Promise((resolve) => {
...
;(function () {
  ...
  suite.skip('client 1 execution', async function () {
  ...
  suite.skip('client 2 execution', async function () {
...
;(function () {
  ...
  const setupZoomTable = function (cb) {
    client.query(
      'CREATE TABLE IF NOT EXISTS zoom(spanner_pk_id BIGINT GENERATED BY DEFAULT AS IDENTITY (BIT_REVERSED_POSITIVE) PRIMARY KEY, name varchar(100));',
      function (err) {
        assert.ifError(err)
        client.query('DELETE FROM zoom WHERE 1=1', function (err) {
          assert.ifError(err)
          client.query(
            "INSERT INTO zoom (name) VALUES ('zed'), ('postgres'), ('node postgres')",
            cb
          )
        })
      }
  ...
  suite.skip('with small row count', function (done) {
    setupZoomTable(function () {
      const query = client.query(
        new Query(
          {
            name: 'get names',
  ...
  suite.skip('with large row count', function (done) {
    const query = client.query(
      new Query(
        {
          name: 'get names',
  ...
  suite.skip('with no data response and rows', async function () {
