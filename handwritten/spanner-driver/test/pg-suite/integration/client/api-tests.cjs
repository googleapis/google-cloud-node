...
suite.test('callback is fired once and only once', function (done) {
  ...
  pool.connect(
    assert.calls(function (err, client, release) {
      assert(!err)
      client.query('CREATE TABLE IF NOT EXISTS boom_api_test(spanner_pk_id BIGINT GENERATED BY DEFAULT AS IDENTITY (BIT_REVERSED_POSITIVE) PRIMARY KEY, name varchar(10))', function (err) {
        assert.ifError(err)
        let callCount = 0
        client.query(
          [
            "INSERT INTO boom_api_test(name) VALUES('hai')",
            "INSERT INTO boom_api_test(name) VALUES('boom')",
            "INSERT INTO boom_api_test(name) VALUES('zoom')",
          ].join(';'),
