...
const Client = require('./../../build/cjs/src/index.cjs').Client
...
const expect = function (callback, timeout) {
  ...
  timeout = timeout || parseInt(process.env.TEST_TIMEOUT) || 120000
...
const createPersonTable = async (client) => {
  await client.query('CREATE TABLE IF NOT EXISTS person (id bigint generated by default as identity (bit_reversed_positive) primary key, name varchar(10), age integer)')
...
module.exports = {
  Suite: Suite,
  pg: require('./../../build/cjs/src/index.cjs'),
  args: { native: false },
  config: {
    native: false,
