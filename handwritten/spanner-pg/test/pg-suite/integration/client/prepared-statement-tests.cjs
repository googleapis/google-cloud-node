...
;(function () {
  const client = helper.client()
  client.query('CREATE TABLE IF NOT EXISTS zoom(spanner_pk_id BIGINT GENERATED BY DEFAULT AS IDENTITY (BIT_REVERSED_POSITIVE) PRIMARY KEY, name varchar(100));', function (err) {
              assert.ifError(err)
