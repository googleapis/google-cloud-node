...
// Raw TCP socket mock servers writing PostgreSQL wire binary packets (test-buffers.js)
// are unsupported in @google-cloud/spanner-pg as it communicates via gRPC / spannerlib channels.
suite.skip('readyForQuery server', (done) => {
  const respondingServer = new Server(buffers.readyForQuery())
  testServer(respondingServer, done)
})

suite.skip('silent server', (done) => {
