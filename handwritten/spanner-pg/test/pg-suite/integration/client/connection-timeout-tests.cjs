...
// Cloud Spanner Note: connection-timeout-tests.cjs creates a raw TCP net.Server mock that writes binary
// PostgreSQL wire-protocol bytes (test-buffers.js).
// @google-cloud/spanner-pg communicates via gRPC / spannerlib channels to Cloud Spanner rather than raw TCP sockets.

suite.skip('successful connection', (done) => {
...
suite.skip('expired connection timeout', (done) => {
