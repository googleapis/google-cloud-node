...
describe.skipClassic('Pipeline window functions (count)', () => {
  let firestore: Firestore;
  ...
  before(async () => {
    productSales = getTestRoot();
    await testCollectionWithDocs(productSales, windowTestDocs);
    firestore = productSales.firestore;
  });
  ...
  describe('error handling', () => {
    it('rejects a range frame without a sort', async () => {
      ...
      ).to.be.rejectedWith(Error, /range/i);
    });
    ...
    it("rejects specifying both 'documents' and 'range'", async () => {
      ...
      ).to.be.rejectedWith(Error, /documents.*range|range.*documents/i);
    });
    ...
    it('rejects a partition supplied to an accumulator level over()', async () => {
      ...
      ).to.be.rejectedWith(Error, /unexpected field/i);
    });
