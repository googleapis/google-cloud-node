...
import {Storage, File, Bucket} from '../src/index.js';
...
const argv = yargs(process.argv.slice(2))
  ...
  .option('concurrency', {
    ...
    alias: 'c',
    default: 1,
    description: 'Number of concurrent operations to run in parallel (default: 1)'
  })
  .option('baseline', {
    type: 'string',
    description: 'Baseline version of @google-cloud/storage to compare against (e.g., 7.19.0)'
  })
  .option('fileSize', {
    type: 'number',
    default: 1024,
...
async function cleanupResources(resources: Array<{delete(): Promise<any>}>, concurrency = 32) {
...
async function runUploadScenario(
  ...
  console.log(`Starting Scenario: Upload (${argv.fileSize} bytes, concurrency: ${argv.concurrency})...`);
  const options = argv.resumable !== undefined ? {resumable: argv.resumable} : {};
  ...
  return runConcurrent(
    ...
    (completed) => logMemory(`  Upload completed ${completed}/${argv.iterations}`)
  );
}
...
async function runStreamUploadScenario(
  ...
  console.log(`Starting Scenario: Stream Upload (${argv.fileSize} bytes, concurrency: ${argv.concurrency})...`);
  const options = argv.resumable !== undefined ? {resumable: argv.resumable} : {};
  ...
  return runConcurrent(
    ...
    (completed) => logMemory(`  Stream Upload completed ${completed}/${argv.iterations}`)
  );
}
...
async function runLocalFileUploadScenario(
  ...
  console.log(`Starting Scenario: Local bucket.upload() (${argv.fileSize} bytes, concurrency: ${argv.concurrency})...`);
  
  const localFilePath = path.join(os.tmpdir(), `bench-local-${name}-${Date.now()}.bin`);
  ...
  try {
    const resumableTimes = await runConcurrent(
      ...
      async (i) => {
        ...
        const start = performance.now();
        const [resFile] = await bucket.upload(localFilePath, { destination: resName, resumable: true });
        const duration = performance.now() - start;
        uploadedFiles.push(resFile);
        return duration;
      },
      (completed) => logMemory(`  Local Resumable Upload completed ${completed}/${argv.iterations}`)
    );
    ...
    const multipartTimes = await runConcurrent(
      ...
      async (i) => {
        ...
        const start = performance.now();
        const [multiFile] = await bucket.upload(localFilePath, { destination: multiName, resumable: false });
        const duration = performance.now() - start;
        uploadedFiles.push(multiFile);
        return duration;
      },
      (completed) => logMemory(`  Local Multipart Upload completed ${completed}/${argv.iterations}`)
    );
...
async function runMetadataScenario(
  mainFile: File
): Promise<number[]> {
  console.log(`Starting Scenario: Get Metadata (concurrency: ${argv.concurrency})...`);
  return runConcurrent(
    argv.iterations,
    argv.concurrency,
    async () => {
    ...
    (completed) => logMemory(`  Metadata completed ${completed}/${argv.iterations}`)
  );
}
...
async function runDownloadScenario(
  ...
  console.log(`Starting Scenario: Download (${argv.fileSize} bytes, concurrency: ${argv.concurrency})...`);
  return runConcurrent(
    argv.iterations,
    argv.concurrency,
    async () => {
    ...
    (completed) => logMemory(`  Download completed ${completed}/${argv.iterations}`)
  );
}

async function runStreamDownloadScenario(
  mainFile: File
): Promise<number[]> {
  console.log(`Starting Scenario: Stream Download (${argv.fileSize} bytes, concurrency: ${argv.concurrency})...`);
  return runConcurrent(
    argv.iterations,
    argv.concurrency,
    async () => {
    ...
    (completed) => logMemory(`  Stream Download completed ${completed}/${argv.iterations}`)
  );
}
...
async function runFileGetSaveAndResumableCreateScenario(
  ...
  mainFile: File,
  content: Buffer
): Promise<{ getTimes: number[]; createResumableTimes: number[]; saveMultipartTimes: number[] }> {
  console.log(`Starting Scenario: File .get(), save(multipart), and createResumableUpload() (concurrency: ${argv.concurrency})...`);
  ...
  try {
    const results = await runConcurrent(
      ...
      async (i) => {
        ...
        const multiFile = bucket.file(`bench-save-multi-${Date.now()}-${i}.bin`);
      ...
      (completed) => logMemory(`  Missing Methods completed ${completed}/${argv.iterations}`)
    );
...
async function runListFilesScenario(
  ...
  console.log(`Starting Scenario: List Files (getFiles & getFilesStream, concurrency: ${argv.concurrency})...`);
  return runConcurrent(
    argv.iterations,
    argv.concurrency,
    async () => {
    ...
    (completed) => logMemory(`  List Files completed ${completed}/${argv.iterations}`)
  );
}

async function runExistsScenario(
  mainFile: File
): Promise<number[]> {
  console.log(`Starting Scenario: Exists (concurrency: ${argv.concurrency})...`);
  return runConcurrent(
    argv.iterations,
    argv.concurrency,
    async () => {
    ...
    (completed) => logMemory(`  Exists completed ${completed}/${argv.iterations}`)
  );
}
...
async function runSetMetadataScenario(
  ...
  console.log(`Starting Scenario: Set Metadata (concurrency: ${argv.concurrency})...`);
  const tempFiles: File[] = [];
  try {
    await runConcurrent(
      argv.iterations,
    ...
    return await runConcurrent(
      ...
      (completed) => logMemory(`  Set Metadata completed ${completed}/${argv.iterations}`)
    );
  } finally {
    await cleanupResources(tempFiles);
  }
...
async function runDeleteScenario(
  ...
  console.log(`Starting Scenario: Delete (concurrency: ${argv.concurrency})...`);
  ...
  return runConcurrent(
    ...
    (completed) => logMemory(`  Delete completed ${completed}/${argv.iterations}`)
  );
}
...
async function runBucketLifecycleScenario(
  ...
  console.log(`Starting Scenario: Bucket Lifecycle (Create, Get, Exists, Delete, concurrency: ${Math.min(argv.concurrency, 8)})...`);
  ...
  return runConcurrent(
    ...
    (completed) => logMemory(`  Bucket Lifecycle completed ${completed}/${argv.iterations}`)
  );
}
...
async function runBucketPatchScenario(
  ...
  try {
    return await runConcurrent(
      ...
      (completed) => logMemory(`  Bucket Patch completed ${completed}/${argv.iterations}`)
    );
  } finally {
...
async function runBucketLockScenario(
  ...
  try {
    return await runConcurrent(
      ...
      (completed) => logMemory(`  Bucket Lock completed ${completed}/${argv.iterations}`)
    );
  } finally {
    await bucket.delete().catch(() => {});
  }
...
async function runStorageListAndAccountScenario(
  ...
  console.log(`Starting Scenario: Storage List and Service Account (concurrency: ${argv.concurrency})...`);
  return runConcurrent(
    argv.iterations,
    argv.concurrency,
    async () => {
    ...
    (completed) => logMemory(`  Storage List/Account completed ${completed}/${argv.iterations}`)
  );
}
...
async function runFilePatchAndAclScenario(
  ...
  console.log(`Starting Scenario: File Patch, Get, and ACL (concurrency: ${argv.concurrency})...`);
  return runConcurrent(
    argv.iterations,
    argv.concurrency,
    async (i) => {
      ...
      try {
        ...
        if (i === 0) {
          console.warn('    [Skip] Bucket likely has Uniform Bucket-Level Access enabled. Skipping ACL benchmark.');
        }
        return 0;
      }
    }
...
async function runFileCopyMoveComposeScenario(
  ...
  console.log(`Starting Scenario: File Copy, Move, Rename, Rotate Key, Storage Class and Compose (concurrency: ${argv.concurrency})...`);
  const tempFiles: File[] = [];
  ...
  try {
    return await runConcurrent(
      ...
      (completed) => logMemory(`  File Copy/Move/Compose completed ${completed}/${argv.iterations}`)
    );
  } finally {
    await cleanupResources(tempFiles);
  }
...
async function runNotificationScenario(
  bucket: Bucket,
  name: string
): Promise<number[]> {
  console.log(`Starting Scenario: Notifications (concurrency: ${argv.concurrency})...`);
  ...
  try {
    return await runConcurrent(
      ...
      (completed) => logMemory(`  Notification completed ${completed}/${argv.iterations}`)
    );
  } finally {
...
async function runHmacKeyScenario(
  storage: Storage
): Promise<number[]> {
  console.log(`Starting Scenario: HMAC Key Management (concurrency: ${argv.concurrency})...`);
  ...
  try {
    ...
    return await runConcurrent(
      ...
      async () => {
        const start = performance.now();
        try {
          const [hmacKey] = (await storage.createHmacKey(email)) as any;
      ...
      (completed) => logMemory(`  HMAC Key completed ${completed}/${argv.iterations}`)
    );
  } catch (err) {
    console.warn('    [Warning] HMAC Scenario initialization failed (could not fetch service account). Skipping.');
    return [];
  } finally {
...
async function runBucketIamScenario(
  bucket: Bucket
): Promise<number[]> {
  console.log(`Starting Scenario: Bucket IAM (getIamPolicy, setIamPolicy, testIamPermissions)...`);
  ...
  return runConcurrent(
    ...
    (completed) => logMemory(`  Bucket IAM completed ${completed}/${argv.iterations}`)
  );
}
...
async function runBenchmark(StorageClass: typeof Storage, name: string, bucketName: string) {
  ...
  try {
    const uploadTimes = await runUploadScenario(bucket, content, safeName, uploadedFiles);
    reportResults(`Upload (${argv.fileSize} bytes)`, uploadTimes, true);
    logMemory('After Upload');

    const streamUploadedFiles: File[] = [];
    const streamUploadTimes = await runStreamUploadScenario(bucket, content, safeName, streamUploadedFiles);
    reportResults(`Stream Upload (${argv.fileSize} bytes)`, streamUploadTimes, true);
    logMemory('After Stream Upload');
    uploadedFiles.push(...streamUploadedFiles);

    const localUploadResults = await runLocalFileUploadScenario(bucket, content, safeName, uploadedFiles);
    reportResults('Local bucket.upload() Resumable', localUploadResults.resumableTimes, true);
    reportResults('Local bucket.upload() Multipart', localUploadResults.multipartTimes, true);
    ...
    const fileGetSaveCreateResults = await runFileGetSaveAndResumableCreateScenario(bucket, mainFile, content);
    reportResults('File .get()', fileGetSaveCreateResults.getTimes);
    reportResults('File .save({ resumable: false })', fileGetSaveCreateResults.saveMultipartTimes, true);
    reportResults('File .createResumableUpload()', fileGetSaveCreateResults.createResumableTimes);
    ...
    reportResults(`Stream Download (${argv.fileSize} bytes)`, streamDownloadTimes, true);
    ...
    try {
      const bucketLifecycleTimes = await runBucketLifecycleScenario(storage, safeName);
      reportResults('Bucket Lifecycle', bucketLifecycleTimes);
    } catch (err) {
      console.warn('    [Warning] Bucket Lifecycle scenario failed (likely missing storage.buckets.create permissions). Skipping.');
    }
    ...
    try {
      ...
      console.warn('    [Warning] Bucket Patch scenario failed (likely missing storage.buckets.create permissions). Skipping.');
    }
    ...
    try {
      ...
      console.warn('    [Warning] Bucket Lock scenario failed (likely missing storage.buckets.create permissions). Skipping.');
    }
    ...
    try {
      ...
      console.warn('    [Warning] Storage List & Service Account scenario failed (likely missing storage.buckets.list permissions). Skipping.', err);
    }
    logMemory('After Storage List/Account');

    try {
      const filePatchAclTimes = await runFilePatchAndAclScenario(bucket, mainFile);
      if (filePatchAclTimes.length > 0) {
        reportResults('File Patch, Get, and ACL', filePatchAclTimes);
      }
    } catch (err) {
      console.warn('    [Warning] File Patch, Get, and ACL scenario failed. Skipping.');
    }
    logMemory('After File Patch/ACL');

    try {
      const fileCopyMoveComposeTimes = await runFileCopyMoveComposeScenario(bucket, mainFile, safeName);
      reportResults('File Copy, Move, Compose & Storage Class', fileCopyMoveComposeTimes);
    } catch (err) {
      console.warn('    [Warning] File Copy, Move, Compose & Storage Class scenario failed. Skipping.', err);
    }
    logMemory('After File Copy/Move/Compose');

    const notificationTimes = await runNotificationScenario(bucket, safeName);
...
async function main() {
  try {
    ...
    if (argv.concurrency < 1) {
      throw new Error('Concurrency parameter must be greater than or equal to 1');
    }
    if (argv.fileSize < 0) {
