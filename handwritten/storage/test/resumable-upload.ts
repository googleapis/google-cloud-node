...
describe('resumable-upload', () => {
  ...
  describe('invocationId option', () => {
    const INVOCATION_ID = 'test-invocation-id-9999';

    it('should use the provided invocationId for createURI and not rotate it', async () => {
      ...
      let called = false;
      uploadInstance.makeRequest = async (opts: GaxiosOptions) => {
        called = true;
        const googAPIClient = (opts.headers as Record<string, string>)['x-goog-api-client'];
        assert.ok(googAPIClient.includes(`gccl-invocation-id/${INVOCATION_ID}`));
        return {
          headers: {
            get: () => 'http://fake-location',
          },
        } as unknown as GaxiosResponse;
      };

      await uploadInstance.createURI();
      assert.strictEqual(called, true);
      assert.strictEqual(uploadInstance.currentInvocationId.uri, INVOCATION_ID);
    });

    it('should use the provided invocationId for checkUploadStatus and not rotate it', async () => {
      const uploadInstance = upload({
        bucket: BUCKET,
        file: FILE,
        metadata: METADATA,
      ...
      uploadInstance.makeRequest = async (opts: GaxiosOptions) => {
        ...
        const googAPIClient = (opts.headers as Record<string, string>)['x-goog-api-client'];
        assert.ok(googAPIClient.includes(`gccl-invocation-id/${INVOCATION_ID}`));
        return {} as GaxiosResponse;
      };

      await uploadInstance.checkUploadStatus();
      assert.strictEqual(called, true);
      assert.strictEqual(uploadInstance.currentInvocationId.checkUploadStatus, INVOCATION_ID);
    });

    it('should use the provided invocationId for chunk uploads and not rotate it', async () => {
      const uploadInstance = upload({
        bucket: BUCKET,
        file: FILE,
        metadata: METADATA,
      ...
      let called = false;
      uploadInstance.makeRequestStream = async (opts: GaxiosOptions) => {
        called = true;
        const googAPIClient = (opts.headers as Record<string, string>)['x-goog-api-client'];
        assert.ok(googAPIClient.includes(`gccl-invocation-id/${INVOCATION_ID}`));
        // Return a mock response that allows responseHandler to run successfully
        return {
          status: 200,
          data: {},
          headers: {},
      ...
      uploadInstance.offset = 0;
      uploadInstance.writeBuffers = [Buffer.from('hello')];
      uploadInstance.upstreamEnded = true;

      await uploadInstance.startUploading();
      assert.strictEqual(called, true);
      assert.strictEqual(uploadInstance.currentInvocationId.chunk, INVOCATION_ID);
    });
  });
});
