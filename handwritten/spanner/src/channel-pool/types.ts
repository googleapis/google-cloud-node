...
export interface ChannelPool {
  ...
  /**
   * Returns the underlying gRPC channels currently managed by the pool.
   */
  getChannels(): grpc.Channel[];

  /** Total number of currently in-flight RPCs across all channels in the pool. */
  readonly totalInFlight?: number;

  /** Total number of active Read/Write transactions across all channels in the pool. */
  readonly totalActiveRw?: number;
}
...
export interface DynamicChannelPoolOptions {
  ...
  /** Low-load threshold per channel used for scale-down checks. Defaults to 2. */
  minRpcPerChannel?: number;
  /** Maximum percentage of current pool size added per scale-up event. Defaults to 100 (min 2 channels). */
  maxScaleUpPercent?: number;
  /** Maximum number of channels marked draining per scale-down cycle. Defaults to 4. */
  ...
  /** Cooldown in milliseconds between consecutive scale-up attempts. Defaults to 1,000ms. */
